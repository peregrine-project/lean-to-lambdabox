...
them, a measured 33-name axiom footprint (Assumption~\ref{asm:lean-reflection-axioms}), and
sixteen inherited lean4lean \code{sorry} sites (Assumption~\ref{asm:lean4lean-trust}).
\item \lead{Status} \stAssumed{} on every class-C/D/upstream node; \stInherited{} on the lean4lean
and reflection-axiom nodes; \stOpen{} on the two \code{ErasureBridge} fields, the sole open proof
obligation.
...
\ref{asm:EraserAsks-kernel_ind_head_true} & C & every rung (false in general) & \stAssumed{} \\
\ref{asm:EraserAsks-block_keys_distinct} & C & every rung (false in general) & \stAssumed{} \\
...
\item \lead{Class} C; \lead{Stands at} every rung, discharged nowhere --- and \textbf{false in
general}: the arity-check fuel saturates at 256 (an 8-bit depth field) and is \textbf{1} at a
definitional alias, where the depth is zero (finding F-DEPTH). Measured true at 2{,}940 of 2{,}940
inductive type formers of the elaboration environment.
...
list.
\begin{itemize}
\item \lead{Class} C; \lead{Stands at} every rung, discharged nowhere --- and \textbf{false} on a
legal \code{mutual unsafe def} pairing a definition with its own \code{\_unsafe\_rec} companion:
one key for two members, no error (finding F-UNSAFEREC). Measured: 0 failures over 228{,}937
\code{getDeclInfo?} answers.
\item \lead{Would be discharged by} F-UNSAFEREC's one-line \code{Nodup} guard.
\end{itemize}
\end{assumption}
...
\begin{remark}
\lead{Caveat} \code{bridgeEnv\_of\_regInv} is \emph{proved}, at nine axioms and no
...
\item The delta arm brings in no instantiation cluster: the environment relation's definition
clause is already quantified over it.
...
\textbf{unsatisfiable} hypothesis and were vacuous; under the current test the offending-name
count is 0 at all eight, recomputed at every regeneration.
...
Policy: findings are raised on a separate branch and never patched on the verification branch, so
the theorems are about the code as it ships.
...
\begin{itemize}
\item \lead{Feeds this blueprint's claims}: F-ETA is why expandedness is not a capstone
conjunct; F-DEPTH and F-UNSAFEREC are the general refutations of
...
proved here; the fragment excludes colliding inputs by its own check.
\item \lead{One patch}, the sole exception to the never-patch rule: the arity-check fuel loop now
throws on exhaustion instead of returning \code{false}, reproducing the pre-reroute verdict via
the \code{MetaM} fallback --- verification-authored code, hence patched.
\end{itemize}
...
program the eraser otherwise accepts --- sparse-\code{casesOn}, queued as F-SPARSE.
\end{remark}
...
\begin{itemize}
\item \lead{The content clause}. All remaining proof work is \hl{one item}: the two fields of
...
at the run's final state. The repair plan: one content clause carrying both readings of a
\emph{single} erasure witness, read at a specification environment threaded as an \emph{output} of
the refinement, closing the determinism gap ($\Erases$ is not deterministic outside the
first-order fragment). Assumption~\ref{asm:hargReach} at G8 is the same obligation from the ladder.
\item \lead{Obstruction (i): level scope.} The refinement's eighteen motives are stated at a fixed
level scope; the mutual-block pass re-enters under the member's own parameters, and 16 of G7's 30
tabled bodies are polymorphic.
\item \lead{Obstruction (ii): alpha.} The table's body clause pins a tabled body only up to
alpha-equivalence, while $\Lower$ reads a declared body by equality; five of G7/G8's bodies match
only up to binder names.
\item \lead{Obstruction (iii): the environment quantifier.} The refinement's run predicate reads
content at \emph{every} specification environment of the final state; the repair produces
\emph{one}, unifiable only by restating all eighteen motives.
...
\item Two \code{EraserAsks} fields are \emph{false in general}, holding only on the measured
inputs (Assumptions~\ref{asm:EraserAsks-kernel_ind_head_true}, \ref{asm:EraserAsks-block_keys_distinct}).
Every statement taking a bundle \code{E : EraserAsks} --- the capstone and all eight rungs --- is
bounded by them.
