...
These eight rules are the composite's introduction form. Read against [S, Fig. 18], the \code{box}, \code{app}, \code{lit}, \code{proj} and \code{ctor} rules are the erasure rules transported to \code{Lean.Expr}; \code{fix} and \code{cases} exist only because Lean's kernel has no \code{tFix} and no \code{tCase}, and are where a Lean frontend departs from an \code{EAst}-shaped source. The nineteen results of this and the next section are measured individually in the committed fixture \code{test/erasesLB.expected}, each with footprint \code{[propext, Classical.choice, Quot.sound]} --- \textbf{no} \code{sorryAx}, even though several mention lean4lean's \code{TrExprS}: mentioning a lean4lean \emph{definition} costs nothing, only its proved theorems carry the inherited sorries.
