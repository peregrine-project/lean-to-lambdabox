...
Two module doc-comments and the project's own status notes disagree on how to count the
premises (\code{ErasesCorrect/Steps.lean}: ``seven binders carrying six premises \dots{} plus
the eighth''; \code{ErasesCorrect/Close.lean}: ``eight binders and seven premises'';
\code{doc/rework/07-STATUS.md}: ``eight premises''); all three resolve to the eight binders
above.
\end{remark}
...
\code{Erases.mkApps\_forall}$_2$, whose pointwise premise is a \code{List.Forall}$_2$ witness.
\end{lemma}
\begin{proof}
\leanok
\uses{def:Erases}
...
so --- booked neither to Letouzey nor to MetaRocq's \code{firstorder\_ind}. The trust documentation
(\code{doc/upstream-asks.md}, \code{doc/trust.md}) records that MetaRocq's shipped
\code{firstorder\_ind} evaluates to false on \code{nat}, so this development cites rather than
transcribes it.
\end{remark}
