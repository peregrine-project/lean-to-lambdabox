...
MetaRocq's own sources are cited by file wherever this blueprint itself makes a rule-by-rule
comparison to them, since [S] does not display the \lbox{} evaluation relation at all:
\code{EAst}, in Chapter~\ref{chap:lambdabox}. \code{EWcbvEval} is instead named by identifier
throughout and never by file, as are \code{PCUICFirstorder} and \code{EOptimizePropDiscr}, whose
file-name citations live in the repository's own findings (\code{doc/upstream-asks.md},
\code{doc/rework/}) rather than in this blueprint; \code{ExAst} names ConCert's \emph{typed}
\lboxT{} AST and has no rule-by-rule comparison anywhere in this untyped development. The on-disk
