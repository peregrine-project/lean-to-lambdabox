...
\code{Erasure.visitMutual}). The last two fields are allowed to \emph{assume} the shape of the term
being stored --- fix-free, closed at level $0$, applied form --- because those facts are false of an
arbitrary term and are proved by the same induction that consumes them.
\end{definition}
\begin{remark}
...
--- none on the state, none on the source expression, none on the configuration --- and the sixteen
\code{panic!} arms of the eraser are discharged rather than excluded. Three thin corollaries project
