...
\item \lead{Paper} [S, \S7.1, \S7.4] for \WcbvEval{}; Letouzey's amendments [L, Def.~5, Def.~8] for the box and singleton-case rules.
\end{itemize}
...
\begin{remark}
\lead{Deviation} Against \code{E.term} of [S, Fig.~16]: \code{.fvar} is added (locally nameless, [R, \S4.7]), branches carry binder-name lists ([S, Fig.~18]), and \code{tCoFix} is dropped (\code{.prim} mirrors MetaRocq's own later addition, not a fourth change). Block vs.\ applied form is a \emph{flag} (Section~\ref{sec:lambdabox:flags}); the eraser emits applied form.
\end{remark}
...
\leanok
\lead{In short} A triple of Booleans parameterising weak call-by-value evaluation, matching MetaRocq's \code{EWcbvEval.WcbvFlags} [S, \S7.4].
\begin{center}
...
\begin{remark}
\lead{Why} The two spine clauses are stated \textbf{structurally over \code{.app}}, spine shape carried as a premise equation, so \code{Value} stays invertible by case analysis; MetaRocq indexes directly by $\mkApps f\ args$. Compare [S, Fig.~12].
\end{remark}
...
\lead{In short} The target semantics of the whole verification, $\Gamma \vdash t \Downarrow_{fl} v$: \hl{twenty-two rules}, the Lean counterpart of MetaRocq's \code{EWcbvEval.eval} [S, \S7.1].
\begin{center}
...
\item \textbf{Prop-case is inert on emitted output.} A measurement, not a theorem: the shipping registration never sets \code{propositional} (F-PROP), \code{kelim}, or the recursivity kind.
