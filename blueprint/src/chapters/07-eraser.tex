...
Theorem~\ref{thm:Vacuity-not_upstreamAsks_natEnv} (the classification ask is refuted at any
inductive-bearing environment).
\item \lead{Status} \stProved{} the oracle reroute; \stAssumed{} seven \code{ErasureSpec} fields
(class D), four \code{EraserAsks} fields (class C, one not a theorem); \stInherited{} through
...
\item \lead{Paper} [JACM, Sec.~7.3] for the eraser; [JACM, Sec.~7.4] for \S\ref{sec:eraser:origin}'s
case-step premises.
\end{itemize}
...
table; the accumulating \lbox{} environment; kernames hinted for inlining; the \lbox{} key
minted for each registered mutual inductive block, paired with the block's own member list, so a
second block reaching the same key is caught rather than silently overwriting the first
(Definition~\ref{def:Erasure-checkKernameFresh}).
...
[JACM, Sec.~7.3]'s test, \hl{re-implemented on lean4lean's verified kernel} instead of Lean's
elaborator.
\begin{itemize}
...
\end{itemize}
\lead{Lean} \code{isArityCheck} starts the loop at a fixed fuel of $100{,}000$, not a fuel derived
from the unreduced type's syntactic depth.
\end{definition}

\begin{remark}
\lead{Caveat} A fixed fuel decides every telescope of fewer than $100{,}000$ binders and every
definitional alias inside the kernel, where the old depth-derived fuel (8 bits, saturating at
256, and zero behind an \code{@[irreducible]} alias) could exhaust early and fall back. What a
constant budget cannot bound is a \emph{reduced} telescope longer than it, or any other kernel
error \code{isArityCheck} raises: both still route to the elaborator fallback and still leave
Theorem~\ref{thm:EraserAsks-oracle_informative}'s bundle field
(\code{kernel\_ind\_head\_true}) short of a theorem.
\end{remark}
...
(Theorem~\ref{thm:Oracle-kernel_isErasable_sound}); the fallback arm --
\code{ErasureSpec.oracle\_refl}'s second disjunct, \code{Oracle.MetaSound} -- is assumed sound,
empirically dead over $139{,}196$ constants, though that measures only the error route.
\end{remark}
...
loop at the fixed fuel of Definition~\ref{def:isErasable}.
\lead{Lean} both quantify universally over the fuel, so a fixed budget in the entry point needs no
change to either statement.
\end{lemma}
\begin{proof}
\leanok
...
\uses{def:lean4lean-grounding}
\lean{Lean4Lean.TypeChecker.VContext.ofMLCtx, LeanToLambdaBox.kernelNGen, Lean4Lean.TypeChecker.VContext.ofMLCtx1_venv, Lean4Lean.TypeChecker.VContext.ofMLCtx1_lparams, Lean4Lean.TypeChecker.VContext.ofMLCtx1_mlctx}
\leanok
...
\begin{itemize}
\item \code{kernelNGen}: this repository's own name (\code{CheckerAdequacy.lean}) for the fork's
initial checker state's name generator, which the fork spells out inline rather than naming;
\emph{reserves} every free variable not of the shape \code{\_kernel\_fresh i} -- every
\code{\_uniq}-named identifier a real run produces;
\item all other fields are taken over unchanged.
\end{itemize}
\lead{Lean} three simp projections record the built context's environment, level parameters and
modelled local context (\code{mlctx}, of which \code{vlctx} is an \code{abbrev}) as the given
ones.
\end{definition}

\begin{remark}
\lead{Caveat} This repository names nothing inside \code{namespace Lean4Lean.TypeChecker}: the
six fork names here and in Lemma~\ref{lem:Lean4Lean-TypeChecker-M-WF-run-prime} are cited by their
own qualified names, and only \code{kernelNGen} has a local counterpart. The reservation premise
...
A normalisation pass, key-freshness guards, then registration of inductives and binders into the
emitted environment.
...
\code{@[csimp]}, which \code{ConfigPinned} pins off. \code{remove\_unsafe\_rec} is not injective,
so a legal \code{mutual unsafe def} block can map two members to one \lbox{} key;
\code{visitMutual} refuses to register such a block (Definition~\ref{def:Erasure-checkKernameFresh})
instead of assuming the collision never occurs.
\end{remark}
...
\begin{definition}[Kername freshness guards]
\label{def:Erasure-checkKernameFresh}
\uses{def:Erasure-EraseM, def:Kername}
\lean{Erasure.checkKernameFresh, Erasure.checkIndKernameFresh}
\leanok
\lead{In short} \code{toKername} collapses distinct Lean names to one \lbox{} key (it folds
\code{.num}/\code{.str} components before escaping), so a second registration under a colliding
key would \hl{silently overwrite the first} entry of \code{gdecls}. These two guards throw
instead.
\begin{itemize}
\item \code{checkKernameFresh name kn}: refuses if \code{kn} is already minted by a different
top-level constant, or by a registered inductive block;
\item \code{checkIndKernameFresh names kn}: the block-registering counterpart, refusing if
\code{kn} is already minted by a different block or by a top-level constant.
\end{itemize}
\lead{Lean} called from \code{addAxiom}, \code{addRealizer} and both of \code{visitMutual}'s
registration points, and from \code{register\_inductive}.
\end{definition}

\begin{definition}[The propositional flag, read off the declared arity]
\label{def:Erasure-isPropositionalArity}
\uses{def:Erasure-ErasureConfig}
\lean{Erasure.arityResultSort, Erasure.isPropositionalArity}
\leanok
\lead{In short} \code{isPropositionalArity} decides whether an inductive declared at type
\code{type} lives in \code{Prop}: MetaRocq's \code{isPropositionalArity} (\code{Extract.v:276}),
read syntactically off the sort ending the declared \code{forall}-telescope
(\code{arityResultSort}, MetaRocq's \code{destArity}), with \hl{no reduction}.
\lead{Lean} \code{arityResultSort} walks \code{.forallE}, \code{.letE} and \code{.mdata},
mirroring MetaRocq's \code{destArity}; a telescope hidden behind a \code{.const} alias is still
not read (Theorem~\ref{thm:ErasureSpec-propositionalInd_of_arity}'s caveat).
\end{definition}

\begin{definition}[Registering an inductive block]
\label{def:Erasure-register_inductive}
\uses{def:Erasure-EraseM, def:Erasure-isErasable, def:Kername, def:InductiveId, def:OneInductiveBody, def:Erasure-checkKernameFresh, def:Erasure-isPropositionalArity}
...
\begin{enumerate}
\item synthesise the block's kername from its members' names, concatenated, and refuse a
collision (Definition~\ref{def:Erasure-checkKernameFresh});
...
oracle};
\item set \code{propositional} from the declared arity (Definition~\ref{def:Erasure-isPropositionalArity}),
following MetaRocq's own equation for the field rather than a default;
\item dummy projection names for a single-constructor, single-member, non-recursive block;
\item cons one inductive declaration, carrying the block's parameter count, and record the block's
key and members.
\end{enumerate}
\lead{Lean} \code{kelim} stays at its default: no pass reads it.
\end{definition}

\begin{remark}
\lead{Caveat} \code{Erasure.filter} drops trailing elements, so a short mask silently truncates.
\code{arityResultSort} also walks \code{.letE} and \code{.mdata} (F-ARITYLET), so a \code{let}- or
\code{mdata}-carrying arity is read correctly; a telescope hidden behind a \code{.const} alias
still is not, by design (Theorem~\ref{thm:ErasureSpec-propositionalInd_of_arity}'s caveat).
\end{remark}

\begin{definition}[Realizers: the quotient primitives and a propositional-singleton recursor]
\label{def:Erasure-addRealizer}
\uses{def:Erasure-EraseM, def:Erasure-register_inductive, def:Erasure-isPropositionalArity, def:Kername}
\lean{Erasure.addRealizer, Erasure.quotRealizer, Erasure.recursorRealizer, Erasure.firstNonProofField, Erasure.mkAnonLambdas}
\leanok
\lead{In short} Two ways to register a \lbox{} \emph{body} where Lean's own declaration has none,
in place of the body-less axiom \code{addAxiom} would otherwise register.
\begin{itemize}
\item \code{addRealizer}: \code{addAxiom}'s twin, writing a supplied body instead of \code{none}
(guarded by Definition~\ref{def:Erasure-checkKernameFresh} the same way);
\item \code{quotRealizer}: the identity on \code{Quot.mk} and application on \code{Quot.lift} --
a quotient carries no runtime representation beyond its representative -- \code{Quot} and
\code{Quot.ind} stay $\Box$, being a type former and proof-valued respectively;
\item \code{recursorRealizer}: the \code{.case} body standing in for
\code{remove\_match\_on\_box}'s single-alternative collapse, at the one recursor shape the
collapse may box safely -- a single non-recursive \code{Prop} with at most one constructor, none
of whose fields Definition~\ref{def:Erasure-isPropositionalArity} and
\code{firstNonProofField} let be anything but a proof. \code{Eq.rec}, \code{And.rec} and
\code{False.rec} all take this shape; \code{False.rec}'s case has zero alternatives, since
\code{False} has no constructor to dispatch on.
\end{itemize}
\lead{Lean} \code{firstNonProofField} is the same proof-field scan \code{visitCases}'s Acc guard
runs (\S\ref{sec:eraser:visit}).
\end{definition}
...
\lean{Erasure.mkLambda, Erasure.mkLetIn, Erasure.mkAlt, Erasure.mkDef, Erasure.fvar_to_name, Erasure.withLocalDecl, Erasure.withLocalDef, Erasure.lambdaMonocular, Erasure.lambdaOrIntroToArity, Erasure.etaExpandFix}
\leanok
...
\item \code{etaExpandFix}: wrap a registered recursive member in one anonymous binder applied to
itself, \code{fun x => (fix defs i) x} -- MetaRocq's \code{eta\_fixpoint} at the one arity a
member with principal index zero takes, so a registered fixpoint body is never a bare
\code{.fix} node.
\end{itemize}
\end{definition}
...
all-ASCII form fails at five rungs (Chapter~\ref{chap:capstone}). Two doc comments still hedge on
binder order; Chapters~\ref{chap:lowering},~\ref{chap:refinement} settle it, but the hedge
remains.
\end{remark}

\begin{definition}[Binding already-supplied eta arguments outside the loop]
\label{def:Erasure-withEtaPrefixLets}
\uses{def:Erasure-EraseM, def:Erasure-isErasable, def:Erasure-mkLambda}
\lean{Erasure.withEtaPrefixLets, Erasure.etaArgIsValue}
\leanok
\lead{In short} When an under-applied constructor or eliminator is eta-expanded
(\S\ref{sec:eraser:visit}), an argument already supplied that is \hl{neither a variable nor
erasable} is bound in a \code{let} \emph{outside} the new binders, so it is evaluated once
instead of on every application of the expansion.
\begin{itemize}
\item \code{etaArgIsValue}: true of a variable or an erasable argument -- placing either under
the new binders neither evaluates nor duplicates anything, so it is left in place;
\item \code{withEtaPrefixLets}: given the positions, types and erased values to bind, wraps the
continuation's result in one \code{let} per binding, outermost first.
\end{itemize}
\end{definition}
...
\uses{def:Erasure-EraseM, def:Erasure-isErasable, def:Erasure-register_inductive, def:Erasure-mkLambda, def:Erasure-withEtaPrefixLets, def:LBTerm}
...
\code{visitCasesEtaGo} & eliminator eta-expansion loop, binding a non-value discriminee outside it \\
\code{visitCtorEta} & eta-expand a constructor \\
\code{visitCtorEtaGo} & constructor eta-expansion loop, binding every non-value argument outside it \\
...
shipping code disagrees, and the code wins}.
\end{remark}

\begin{remark}
\lead{Caveat} \code{visitCases} reads the eliminated inductive from \code{casesInfo.indName}
(not the declaration's own name prefix, which for a \code{casesOn} auxiliary generated for a
function is that function's), refuses a per-constructor elimination with a side condition, a
machine-numeral discriminee under a plain \code{casesOn}, and constructor alternatives that do
not correspond one-to-one to the inductive's constructors. On a \hl{propositional inductive with
a non-proof constructor field} (an \code{Acc} clone), it refuses outright rather than emit a
\code{case} that \code{remove\_match\_on\_box} would collapse into a boxed, wrong program. A
sparse \code{casesOn} -- fewer alternatives than constructors, with a catch-all -- expands the
catch-all into one alternative per uncovered constructor, applied to $\Box$ per hypothesis the
catch-all binds; that erasure must have no free de Bruijn index and every hypothesis must be a
proof, both checked rather than assumed.
\end{remark}

\begin{definition}[The environment closure]
\label{def:Erasure-visitMutual}
\uses{def:Erasure-visitExpr, def:Erasure-prepare_erasure, def:Erasure-mkLambda, def:Erasure-EraseM, def:Erasure-checkKernameFresh, def:Erasure-addRealizer, def:Kername, def:GlobalDeclarations}
...
\begin{itemize}
\item no compiler value: a \code{Quot} primitive registers Definition~\ref{def:Erasure-addRealizer}'s
quotient realizer, an eligible recursor its synthesized \code{case} body, and anything else a
body-less axiom;
\item \code{@[extern]} under the axiom policy: a body-less axiom;
\item a lone declaration not mentioning its own name: erase it, refuse a colliding key
(Definition~\ref{def:Erasure-checkKernameFresh}), cons a constant declaration;
\item otherwise: refuse a block whose members map to colliding keys, mint one variable per
member, erase each body under that map, and \hl{register each under its own kername, applied to
its own binder} (Definition~\ref{def:Erasure-mkLambda}'s \code{etaExpandFix}), guarded again by
Definition~\ref{def:Erasure-checkKernameFresh};
\item both re-entries above erase the dependency's body in the \emph{empty} local context at the
dependency's own level parameters, not the caller's (F-DEPLCTX) -- matching MetaRocq's
\code{erase\_constant\_body}, which erases each constant from a pass of its own rather than from
inside the caller's term walk.
\end{itemize}
\end{definition}

\begin{remark}
\lead{Deviation} MetaCoq's \code{erase\_global\_decls}, on demand. \code{SupportError.recursorHead}
and \code{SupportError.quotPrim} still refuse a quotient primitive or a recursor head at
\code{supportedHead}, so a covered program's reachable set stays clear of a realizer key; that a
covered program never registers one through an untabled name is measured, not assumed
(\code{doc/coverage.md}).
\end{remark}
...
constructor, each binding exactly its fields; the information names the eliminated inductive, and
each alternative slot is pinned to its own constructor, in constructor order.
\end{definition}
...
\end{itemize}
\lead{Lean} six fields, one spelled \code{casesOnDecl} (no field may be named \code{casesOn}); a
block's constructor names determine their positions (\code{KernelFields.ctors\_inj}), which is
what lets \code{visitCases}' \code{findIdx?} over the alternatives land on a constructor's own
slot.
\end{definition}

\section{The primitive calls made for their effect}
\label{sec:eraser:prim}
Eight named primitive calls the eraser makes for their effect alone, four of them (the proof
test, the two bounded telescopes, and the shape-crossing clause that carries their bound to the
run lemmas) added for the Acc guard and the sparse catch-all's hypothesis scan.

\begin{definition}[The effectful calls only advance the generator]
\label{def:PrimMonotone}
\uses{def:Erasure-EraseM, def:Erasure-withEtaPrefixLets, def:Erasure-addRealizer, def:MetaGenMono}
\lean{LeanToLambdaBox.PrimMonotone, LeanToLambdaBox.ForallMatchesLam}
\leanok
\lead{In short} The \code{CoreM}/\code{MetaM} calls the erasure makes for their effect alone: each
only advances the name generator. Class D.
\begin{center}
\begin{tabular}{p{0.30\linewidth}p{0.62\linewidth}}
\toprule
Field & What it asserts \\
\midrule
\code{getEnv}, \code{logInfo}, \code{isInstance} & the generator bound alone \\
\code{inferType} & the bound, and a $\forall$-telescope matching the subject's $\lambda$-telescope
(\code{ForallMatchesLam}) \\
\code{isProof} & the bound for \code{Meta.isProof}, the test the Acc guard and the sparse
catch-all's hypotheses run \\
\code{forallBoundedTelescope} & \emph{compositional}: advances the generator no further than the
continuation it is called with \\
\code{lambdaBoundedTelescope} & the same, for the catch-all's telescope \\
\code{liftMetaM} & carries a bound from the shape a \code{MetaM} computation is applied in to the
shape \code{Erasure.liftMetaM} runs it at \\
\bottomrule
\end{tabular}
\end{center}
\lead{Lean} \code{isProof} and the two bounded telescopes are the three named primitives
Definition~\ref{def:MetaGenMono}/\ref{def:PrimGenMono}'s compositional bound is stated over;
\code{liftMetaM} is the one clause that crosses from that applied shape to the shape the run
lemmas read. A single proof-scan lemma discharges both anonymous continuations the erasure passes
to the bounded telescopes: \code{firstNonProofField}'s scan of a constructor's fields and
\code{visitCases}' scan of the catch-all's hypotheses are the same loop, stated once.
\end{definition}

\section{The bundle}
\label{sec:eraser:bundle}

\begin{definition}[The specification of the eraser's primitives]
\label{def:ErasureSpec}
\uses{def:LookupAdequate, def:PrimMonotone, def:BlockAdequate, def:Oracle-MetaSound, def:isErasable, def:Erasure-isErasable, def:lean4lean-grounding, asm:ErasureSpec-env_connect, asm:ErasureSpec-lookup_adequate, asm:ErasureSpec-fresh_names, asm:ErasureSpec-oracle_refl, asm:ErasureSpec-decl_adequate, asm:ErasureSpec-prim_monotone, asm:ErasureSpec-block_adequate}
\lean{LeanToLambdaBox.ErasureSpec}
\leanok
\lead{In short} Seven \stAssumed{} clauses about the world outside pure Lean terms: an elaboration
...
\code{oracle\_refl} & at the ambient scope, a positive verdict reflects a pure kernel run
\emph{or} the assumed-sound fallback \code{Oracle.MetaSound}, both under the same scope premise &
D \\
...
soundness lemmas and the cold-start induction}. \code{oracle\_refl}'s two disjuncts share one
scope premise, \code{ctx.lparams = Us}: the fallback carries no separate scope of its own, so no
transport between two scopes is assumed.
\end{definition}

\begin{remark}
\lead{Caveat} A \emph{definition}: the structure elaborates. Each of its seven fields is a
...
\uses{def:preparePasses, def:Erasure-isErasable, def:isErasable, def:Erasure-prepare_erasure, def:SEval, def:IndInfo, def:Kername, asm:EraserAsks-passes_monotone, asm:EraserAsks-passes_sound, asm:EraserAsks-oracle_false_refl, asm:EraserAsks-kernel_ind_head_true}
\lean{LeanToLambdaBox.EraserAsks}
\leanok
\lead{In short} Four \stAssumed{} clauses about \hl{this repository's own code} -- the
preparation passes and the oracle -- each an obligation with an owner. No field mentions a
reader's level scope: every clause is a statement about a run of the eraser's own code at the
scope that run is made under, so the bundle carries no separate scope parameter.
\begin{center}
\begin{tabular}{p{0.24\linewidth}p{0.61\linewidth}l}
\toprule
Field & What it asserts & Class \\
\midrule
...
\code{oracle\_false\_refl} & a negative verdict: the pure kernel run at the same scope does not
answer positively & C \\
\code{kernel\_ind\_head\_true} & at an inductive-type head the pure run, at the scope it is made
under, answers positively & C \\
\bottomrule
\end{tabular}
\end{center}
\lead{Lean} \code{oracle\_informative} is a \emph{derived theorem}
(Theorem~\ref{thm:EraserAsks-oracle_informative}), not a fifth field.
\end{definition}

\begin{remark}
\lead{Caveat} One field, \code{kernel\_ind\_head\_true}, is not a theorem. \code{isArityCheck}
bounds its arity walk by the fixed budget of Definition~\ref{def:isErasable}, not by the
unreduced type's depth, so a type whose \emph{reduced} telescope still exceeds that budget -- or
any other kernel error the walk raises -- leaves it short of the closing sort and routes the
verdict to the assumed elaborator fallback instead of deciding inside the kernel. True over the
covered corpus ($2{,}940$ of $2{,}940$ type formers, no failure over $200{,}000$-plus
declaration-info answers), so a result using the bundle is conditionally vacuous on such a
counterexample.
\end{remark}
...
\begin{remark}
\lead{Why} Theorem~\ref{thm:ErasureSpec-propositionalInd_of_arity} (Chapter~\ref{chap:trust})
spends \code{decl\_adequate} to show a \code{true} verdict of
Definition~\ref{def:Erasure-isPropositionalArity} sound against the model: the half of MetaRocq's
propositional-flag equation a consumer needs, and the reason
Definition~\ref{def:Erasure-register_inductive} states the field as an equation rather than a
default.
\end{remark}
...
premises of Theorem~\ref{thm:ErasureSpec-oracle_sound_of_run}; both bundle clauses read at the
one scope the run is made under, so no separate scope hypothesis is needed to line them up.
...
\begin{remark}
\lead{Caveat} A theorem, not a fifth field -- the template for retiring fields: derive rather than
assume. Chapter~\ref{chap:refinement} consumes it, but it is exactly as strong as
\code{kernel\_ind\_head\_true}, which is not a theorem, and no stronger.
\end{remark}
...
(constructor, inductive type name, plain constant) must be recovered.
\begin{itemize}
\item \lead{Given} a well-formed environment, \code{consts\_classified} and
\code{constOrigin\_of\_constants} exhibit a declared constant's classification
\emph{unconditionally}, off \code{constOrigin\_of\_wf}: neither spends the upstream ask it still
takes as a binder;
\item \code{constOrigin\_not\_ctorOf}/\code{constOrigin\_not\_indInfo} are the mutual-exclusion
half, and do spend the ask's exclusion conjunct.
\end{itemize}
\lead{Lean} the ask binder in \code{consts\_classified}/\code{constOrigin\_of\_constants} is
unused (a leading underscore); call sites supply it for nothing.
\end{lemma}
\begin{proof}
\leanok
\uses{asm:UpstreamAsks-constsOrigin}
\lead{Idea} \code{consts\_classified}/\code{constOrigin\_of\_constants} project \code{env.WF}
through the environment's own declaration steps; the exclusion pair projects the ask directly.
\end{proof}
...
\begin{theorem}[The classification ask is refuted at any inductive-bearing environment]
\label{thm:Vacuity-not_upstreamAsks_natEnv}
\lean{LeanToLambdaBox.Vacuity.not_upstreamAsks_natEnv, LeanToLambdaBox.Vacuity.natZ_constOrigin, LeanToLambdaBox.Vacuity.natN_constOrigin, LeanToLambdaBox.Vacuity.erases_natZ_two_ways}
\leanok
\uses{def:UpstreamAsks, def:ConstOrigin, def:CtorOf, def:IndInfo, def:Erases}
\lead{In short} \hl{No environment declaring an inductive block satisfies \code{UpstreamAsks}}: at
the fixture environment for \code{Nat}, its exclusion conjunct is false.
\begin{itemize}
\item \lead{Given} the fixture \code{natEnv}, whose block declares \code{Nat};
\item every member of a declared block is also declarable as a plain constant below the
environment (\code{CtorOf.constOrigin}, \code{IndInfo.constOrigin}), so \code{Nat.zero} is both a
constructor and a \code{ConstOrigin} witness there;
\item \lead{Then} \code{UpstreamAsks}'s exclusion conjunct -- a plain constant is neither a
constructor nor a type name -- fails at \code{Nat.zero}, so \code{UpstreamAsks natEnv} has no
witness; matching this, \code{Nat.zero} itself erases two different ways at \code{natEnv}
(\code{Erases.ctor} and \code{Erases.const}), refuting the uniqueness the ask exists to buy.
\end{itemize}
\lead{Lean} every theorem above taking \code{A : UpstreamAsks env} is therefore non-vacuous only
at an \code{env} declaring no inductive at all.
\lead{Status} \stProved{}.
\end{theorem}
\begin{proof}
\leanok
\uses{def:UpstreamAsks}
\lead{Idea} Instantiate the exclusion conjunct at \code{Nat.zero}'s own \code{ConstOrigin}
witness; it demands \code{Nat.zero} not be a constructor, contradicting the fixture's own
\code{CtorOf}.
\end{proof}
...
\begin{proof}
\leanok
\uses{lem:IndInfo-inj, asm:UpstreamAsks-constsOrigin, asm:UpstreamAsks-mkAppsInv, asm:lean4lean-trust}
...
answer survive un-boxed (Chapter~\ref{chap:correctness}). [JACM, Sec.~7.3]'s erasure relation
...
scrutinee land on the right constructor, so the \lbox{} case node picks the matching branch. [JACM,
Sec.~7.4]'s case step.
\end{remark}
...
\begin{proof}
\leanok
\uses{thm:not_erasable_of_informative, asm:lean4lean-trust}
...
The shipping eraser carries ten guards and realizers beyond what [R] describes, each with
its own regression test under \code{test/fixes/}.

\par\smallskip\noindent
\begin{center}\small
\begin{tabular}{p{0.21\linewidth}p{0.22\linewidth}p{0.28\linewidth}l}
\toprule
Guard or realizer & Site & What it does & Test \\
\midrule
Kername freshness (Definition~\ref{def:Erasure-checkKernameFresh}) & \code{addAxiom},
\code{addRealizer}, both of \code{visitMutual}'s exits & refuses a colliding \lbox{} key instead
of silently overwriting the first registration & F-KERNAME \\
Block-key \code{Nodup} guard & \code{visitMutual} & refuses a mutual block whose members map to
colliding keys & F-UNSAFEREC \\
The propositional flag & \code{register\_inductive} & sets \code{propositional} from the declared
arity (Definition~\ref{def:Erasure-isPropositionalArity}) instead of a constant default & F-PROP \\
The Acc guard & \code{visitCases} & refuses eliminating a propositional inductive with a
non-proof constructor field & F-ACC \\
The sparse \code{casesOn} expansion & \code{visitCases} & expands a catch-all into one
alternative per uncovered constructor, checked for a free de Bruijn index and non-proof
hypotheses & F-SPARSE \\
Quotient and recursor realizers (Definition~\ref{def:Erasure-addRealizer}) & \code{visitMutual}'s
axiom arm & synthesises a body for \code{Quot.mk}/\code{Quot.lift}/\code{Quot}/\code{Quot.ind}
and for an eligible propositional-singleton recursor & F-QUOT, F-EQREC \\
Fixed fuel & \code{isArityCheck} & bounds the arity walk by a constant instead of the unreduced
type's syntactic depth & F-DEPTH \\
Eta-wrapped fixpoints (\code{etaExpandFix}) & \code{visitMutual}'s registration loop & registers
a recursive member applied to its own binder, never a bare \code{.fix} node & F-ETA \\
Discriminee/argument-only binding (Definition~\ref{def:Erasure-withEtaPrefixLets}) &
\code{visitCasesEtaGo}, \code{visitCtorEtaGo} & binds an already-supplied non-value argument
outside the eta-expansion's new binders & F-ETA2 \\
Dependency-context reset & \code{visitMutual}'s two \code{withReader} re-entries & resets the
local context to empty alongside the level column, so a closed dependency erases in its own
context, not the caller's & F-DEPLCTX \\
\bottomrule
\end{tabular}
\end{center}
...
\item \lead{Deviation} Every registered recursive fixpoint member is applied to its own binder
(\code{etaExpandFix}), meeting the pipeline's expanded-fixpoint precondition; the capstone
concludes it at every rung.
...
\item \lead{Caveat} One \code{EraserAsks} field, \code{kernel\_ind\_head\_true}, is not a
theorem, so a result using the bundle is conditionally vacuous on a counterexample outside the
covered corpus; \code{auto\_inline\_typeclass\_dispatch} (F-PRODUCT) is unchanged, default off
and pinned false by \code{ConfigPinned}, its disposition an owner decision.
\item \lead{Caveat} \code{panic!} yields $\Box$ and exits successfully, so a refused shape that
slips past a guard would surface as a wrong \code{.ast}, not a failure.
\item \lead{Caveat} \code{UpstreamAsks} is inhabited at no environment declaring an inductive
block (Theorem~\ref{thm:Vacuity-not_upstreamAsks_natEnv}): every theorem above taking it
explicitly is non-vacuous only where none is declared.
\end{itemize}
