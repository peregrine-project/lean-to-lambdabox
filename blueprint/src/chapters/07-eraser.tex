...
\lead{Lean} \code{arityResultSort} walks the \code{.forallE} arm alone; a telescope ending in a
\code{let} is not read (Theorem~\ref{thm:ErasureSpec-propositionalInd_of_arity}'s caveat).
\end{definition}
...
\code{arityResultSort}'s \code{.forallE}-only walk misses an arity ending in a \code{let}-binding,
which MetaRocq's \code{destArity} also walks: such a declaration is emitted non-propositional
although it is propositional (open, \code{doc/rework/03-DEV-FIX.md}, F-ARITYLET).
\end{remark}
...
\code{oracle\_refl} & a positive verdict at the ambient scope reflects a pure kernel run, or the
assumed-sound fallback & D \\
\code{oracle\_meta} & a positive verdict off the ambient scope is sound \emph{at the scope it was
taken under} & D \\
...
soundness lemmas and the cold-start induction}. \code{oracle\_meta}'s scope is exactly
\code{Erasure.isErasable}'s own call-site parameter, \code{ctx.lparams}: a consumer holding a
translation witness at the reader's \code{Us} instead has \code{ctx.lparams = Us} and spends
\code{oracle\_refl}, so no transport between the two scopes is assumed.
\end{definition}
...
The shipping eraser carries nine guards and realizers beyond what [R] describes, each with
