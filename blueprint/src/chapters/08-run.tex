...
say that \textbf{the run of \code{x} from \code{(s, ctx, w)} succeeded with result \code{r}, final
...
the \code{mutual} block in \code{Erasure.lean} --- \code{visitLambda} is second in the source and
...
the inversion (an \code{iff}), and its contrapositive, which discharges every error path of the
eraser.
\end{lemma}
...
\code{modifyGet}, \code{read} and \code{withReader} reduce definitionally once the run is applied
to all five arguments. \code{run\_read\_bind} composes the bind equation with \code{run\_read};
...
declaration entry (Theorem~\ref{thm:Erasure-run_addAxiom_ok}); every binder-helper lemma carries a
...
environment read and log during the run reports \code{Ci} and advances \code{gw} monotonically.
