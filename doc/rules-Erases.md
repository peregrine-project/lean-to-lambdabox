...
| `erases_tLambda` | `lam` | Extends the `VLCtx` exactly as `TrExprS.lam` does. The target binder name is independent of the source's — `Lower.lambda` already quantifies both (`LeanToLambdaBox/Lower.lean:344`), and `WcbvEval` reads binder counts, never names. The ASCII-graphic filter of `fvar_to_name` (`LeanToLambdaBox/Erasure.lean:245-252`) is a printer constraint and lives in `LBWfPeregrine.asciiNames`, not here |
| `erases_tLetIn` | `letE` | ζ must be enabled in both semantics. `letE.nonDep` is ignored; the target binder name is independent of the source's, as in `lam` |
...
| `erases_tProj` | `proj` | Keyed on the source `Expr.proj S i e` and on `IndInfo`, the environment predicate declared alongside the relation: it reads the block off a declaration list of `VEnv.WF'` below `env`, and gives it λ□ coordinates through `indBlockKername` (the eraser's block kername) and `ctorFieldCounts` (Π-binders beyond the parameters). The premise `IndInfo env S iid np [nf]` is the single-constructor gate, and `hi : i < nf` the field-index bound. It carries one premise Fig. 18 does not spell, `hinf : InformativeInd env S`: Rocq's typing forbids a projection out of `Prop`, while Lean's `Expr.proj` does occur there (`h.1` on `And`), and there the discriminant's image may be `.box`, which no rule of `eraseFlags` reduces — the arm would be refutable, not merely unprovable (`erases_proj_needs_informative`). It removes no program: the kernel's projection rule forbids large elimination, so every field of a propositional structure is a proof and `box` is its rule. The criterion is **semantic** — the declared result level never evaluates to zero (`VLevel.IsNeverZero`) — and not the syntactic successor shape, which is false at `Prod` and at ten more corpus projection heads, none of them covered by `box`. `IndDeclOf` is *not* on the rule: it is not monotone in `env`, so `Erases.mono` would fail, and the arm reads it off `ErasesEnv.blocks` at the reached block kername. Deliberately **no** `TrExprS` premise: `TrProj.uniq` yields `IsDefEqU`, not equality, so a term premise would pin parameters the kernel only fixes up to defeq |
...
`IndInfo` comes with `IndInfo.of_wf'` (introduction from a declaration list),
`IndInfo.mono` (the block data survives environment extension, which is what `Erases.mono`
needs at the `proj` arm, alongside `InformativeInd.mono`) and `IndInfo.arity`, the projection
onto `IndArity` — the same block data without the λ□ identifier, which is what a relation
about `Lean.Expr` evaluation reads. `IndDeclOf`, the block read off `env`'s **own**
declaration list, is declared here too, for the consumers that need `env`'s list rather than
one below it. `CtorOf` and `ConstOrigin` come with `.mono` for the same reason,
