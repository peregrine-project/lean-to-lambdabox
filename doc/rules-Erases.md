...
registry. This table is the transport, rule by rule; `lake exe hygiene --tables` checks that
every arm of the Lean inductive appears below.
...
| `erases_tProj` | `proj` | Keyed on the source `Expr.proj S i e` and on `IndInfo`, the environment predicate. Deliberately **no** `TrExprS` premise: `TrProj.uniq` yields `IsDefEqU`, not equality, so a term premise would pin parameters the kernel only fixes up to defeq |
...
| `[S §7.3]` weakening and substitutivity, `[L Lemma 16]` | `erases_shift`, `erases_subst`, `Erases.abstract`, `Erases.uninstantiateN`, `Erases.thin_vlet`, `erases_weakFV*`, `erases_uniform_*` | Statements carried; nine of fifteen arms survive per lemma, six die with the rules that left |
