...
until U3.7 — stay named binders, per A14) and every conclusion ending in a literal peano numeral.
...
| **W2** | β, ζ, literals; the composite; then the re-anchoring the W2 refutations force | 4 + 5 + gate | G2, G3, G4 | in progress |
...
| **G2 gate — green_G2–G4, and the merged `simulate` field** | `Capstone.lean` (merges `ErasureBridge.simulate` and `.lowerCorrect` into the single field `01-DESIGN.md` §5 prints, deletes `erases_mkApps` in favour of `Erases.mkApps`, and folds the spine premise per U2.2's lemma — finding **G1-O3** retired), `VerifyBench/Spikes/{G2,G3,G4}.lean`, `Green.lean`, `test/{ErasesLBCheck.lean,erasesLB.expected}`, `scripts/erasesLB.sh`; N3a files | U2.1–U2.8 **(proof)**, U1.8 **(proof, for `Supported`/`supportedB_sound`)** | 600 | `green_G2`, `green_G3`, `green_G4` elaborate with literal peano answers; **`green_G1` still elaborates** — the standing obligation, re-measured after U2.5–U2.8's rule changes; `hbridge`'s field list matches `01-DESIGN.md` §5 (nine fields, `simulate` merged); `lake exe green-check` covers four rungs; `lake exe hygiene --dup && lake exe hygiene --schedule` exit 0; the ledger fixture re-measured after the `Erases`/`SEval` changes; `doc/coverage.md` updated by hand this wave (the generator is U5.3's; its interim absence is stated in the file header) |

**W2's second half — the re-anchoring its own refutations force.** U2.1 and U2.3 refuted both
halves of the composition strategy (`01-DESIGN.md` §2.3), and two refuters then refuted the first
cut of the re-anchoring (`04-AMENDMENT-W2.md` §2). The five units below land the amended
...
breaks and the chain is U2.9 → U2.5 → {U2.6, U2.7} → U2.8 → G2 (refuter finding B-F4).
`Capstone.lean` stays gate-owned.

| Unit | Files owned | Depends | Est. | Acceptance |
|---|---|---|---|---|
| **U2.9 demolition and relocation — first** | `ErasesCorrect.lean`, `Semantics/Metatheory.lean`, `Closed.lean`, `ErasesAbstract.lean` | U2.1, U2.2, U2.3 **(proof)** | 450 | every statement the amended architecture retires is **deleted with its subject** and recorded in `ErasesCorrect.lean`'s header with the refutation it carried (`erases_correct_tabled`/`_ctx`, `TabledConstants`, `DeltaAgrees`, `erasesEnv_not_deltaAgrees`, `erases_const_ne_construct`, `erases_correct_needs_tabled`, `erases_correct_needs_tabled_ctor`); `Erases.defeqDFC_wt`/`erases_subst_let`/the four `InstLet` helpers elaborate in `ErasesAbstract.lean` and `WcbvEval.{head_value_of_mkApps, app_congr, mkApps_congr, mkApps_box}` in `Semantics/Metatheory.lean`, each with its `#print axioms` unchanged; `erases_mkApps_inv`/`erasable_mkApps` stay (U3.2 consumes them); **the missing target-side lemma lands**: `WcbvEval.lbClosed : ClosedBodies Γ → LBClosed t 0 → WcbvEval Γ fl t v → LBClosed v 0` — no lemma relates `WcbvEval` and `LBClosed` in the tree today (refuter finding B-F5) — with a `_fires` witness; `grep -n "WcbvEval Σ⁺" LeanToLambdaBox/ErasesCorrect.lean` empty; `lake build` green; `lake exe hygiene --dup` = 0 |
| **U2.5 the three readings of `Expr.const`** | `Erases.lean`, `ErasesTotal.lean`, `ErasesAbstract.lean`, `ErasesStrengthen.lean`, `ErasesUniform.lean` | U2.9 **(proof)**, U1.1, U1.2, U1.3 **(proof)** | 700 | `CtorOf` moves into `Erases.lean` beside `IndInfo`; `VDeclDefines`/`ConstOrigin` land as `01-DESIGN.md` §4.2 prints them; `Erases` has eleven arms (`#guard erasesArms%` lists `Erases.ctor`) with `const` re-keyed to `(hc : env.constants c = some ci) (ho : ConstOrigin env c)` — **positive**, so introduction needs no upstream lemma; `ctor_inv` and the three-alternative `const_inv` land; every transport lemma re-elaborates with its measured axiom set; `Erases.exists_of_trExprS_of_projInfo` re-proved, taking the classification as the explicit hypothesis `hclass : ∀ c ci, env.constants c = some ci → (∃ I k, CtorOf env c I k) ∨ (∃ iid np nfs, IndInfo env c iid np nfs) ∨ ConstOrigin env c` (U3.4 discharges it; invent no alternative); `grep -nE "\.case|\.fix" LeanToLambdaBox/Erases.lean` empty; **two** `_fires` witnesses — `ctor` at a one-inductive fixture, and `const` with `ConstOrigin` exhibited from that fixture's own declaration list, which is what keeps `green_G1` reachable this wave; `lake build LeanToLambdaBox.ErasesUniform` green |
| **U2.6 `SEval` = `[S Fig. 12]`'s `value_head`, and the ι split** | `SourceEval.lean`, `SubjectReduction.lean` | U2.5 **(proof)**, U1.4 **(proof)** | 600 | `CasesOnShape` lands as `01-DESIGN.md` §4.3 prints it; `ctorVal` is keyed on `CtorOf` with `hnb`/`hpat`/`hne` **deleted**, `indVal`/`sort`/`forallE` are added, and ι carries `hsh`/`hct`/`hpres`/`hmins` (A20); `SEval.mono`/`le`/`defeq` re-proved with no new binder and the same measured axioms; **four** `_fires` witnesses — ι at a two-branch `casesOn` with `CasesOnShape ``Nat.casesOn ``Nat 1 2` discharged from a concrete `VEnv.WF'` fixture (this is risk R17's test and the earliest point it can fail), `ctorVal` at `Nat.succ`, `indVal` at `Nat` as a type argument, `forallE` inside a δ redex that has **no** derivation without the arm; one refutation witness `untabled_const_not_value` (a body-less plain constant spine has no `SEval` value — the fact that retires `AxiomFree`); `grep -n "IotaInert\|CasesOnOf" LeanToLambdaBox/SourceEval.lean` empty |
| **U2.7 the pass layer, fourteen arms** | `Lower.lean`, `LowerFix.lean`, `LowerCorrect.lean`, `ErasesLB.lean` | U2.9 **(proof)**, U1.5, U1.7 **(proof)** | 1,100 | `Lower` has fourteen arms as `01-DESIGN.md` §4.4 prints them: `ctorApp`, `ctorEta`, `elimEta`, `CtorDecl`, `ElimHeadOf`, `EtaSpine` **deleted**, `RuntimeKey` eliminator-only, `elimApp` `.const`-headed with `ElimDecl`, `fixConst` guarded by `¬ RuntimeKey`, `ElimDecl` carrying its block clause; `LowerCorrect.lean`'s simulation half deleted **in this wave** (`LowerPlain`, `lower_correct_plain`, `lowerFix_correct_plain`, `lower_correct_deltaChain`, `lowerFix_correct_atom`, `LowerNoEta`, `DeltaChain`, `BlockBodiesPlain`, `BlockDefsLambda`, `LowerEnvPlain`) while `BlockBodiesLambda`, the inversion kit, the spine toolkit, `subst_comm` and `substList_comm` move to `Lower.lean`; `Lower.source_const` re-proved with **two** alternatives and `Lower.source_construct_nil` with two; `ErasesLB.ctor_head`/`.ctor` re-derived from `Erases.ctor` + `Erases.app` + `Lower.construct` (`hnul` and `ctor_head_needs_nullary` gone) and the two `*_eta` twins deleted; `LowerBlock.lambda_of_fixLambda` loses `hη`; every deleted refutation recorded in a module header (`constToFix_needs_freshness` **stays** — its subject survives); `test/erasesLB.expected` regenerated, diff handed to the gate; **F-ETA2** filed in `doc/dev-fix-queue.md` with the counterexample and the note that applied-form λ□ needs no constructor η at all (per N5: raised, never patched); `lake build LeanToLambdaBox.ErasesLB` green; `lake exe hygiene --dup` = 0 |
| **U2.8 the environment relation** | `ErasesEnv.lean`, `Output.lean`, `SpecEnv.lean` | U2.5, U2.7 **(proof)**, U1.8, U1.9 **(proof)** | 750 | `ErasesEnv` carries `defns`; `ErasesDecl.ctor` **deleted** and `ErasesDecl.elim` carries `hsh : CasesOnShape env c I dp nfs.length` through the `c`-exposing `CasesOnOf'`; `LowerEnv` gains `specClosed : ClosedBodies Σ⁺` and `specBlocks : BlockBodiesLambda Σ⁺` and `defsTotal` loses its `fix` disjunct (`01-DESIGN.md` §4.8); `constRefs` collects the block kername of `.construct`/`.case`/`.proj`, `ReachableFrom` stays `Decidable`, and **G1's closure is re-measured and printed in the unit report**; `ErasableAxioms`, `AxiomRealizer`, `axiomRealizerNames`, `axiomRealizerB` and `axiomRealizer_iff` **deleted** and **no `AxiomFree` landed** — `grep -rn "ErasableAxioms\|AxiomRealizer\|AxiomFree" LeanToLambdaBox/` empty — with `doc/coverage.md` gaining the F-EQREC row they carried; the threading kit `ReachableFrom.{subterm, app, alt, through_body, substList}` elaborates; the two new `LowerEnv` clauses exhibited on the G1 fixture; build target `lake build LeanToLambdaBox.SpecEnv` green (the tree is red until G2 removes `hax` from `Capstone.lean` and `g1_erasableAxioms` from `Green.lean`, which is that gate's job) |
| **G2 gate — green_G2–G4, the merged `simulate` field, and the axiom-premise removal** | `Capstone.lean`, `Green.lean`, `VerifyBench/Spikes/{G2,G3,G4}.lean`, `test/{ErasesLBCheck.lean,erasesLB.expected,ledger.expected}`, `scripts/erasesLB.sh`; N3a files | U2.1–U2.9 **(proof)**, U1.8 **(proof)** | 650 | `ErasureBridge.simulate` and `.lowerCorrect` merge into the one field `01-DESIGN.md` §5 prints (T9's proof loses an `obtain`); T9 loses `hax` and `Green.lean` loses `g1_erasableAxioms`; `erases_mkApps` deleted in favour of `ErasesLB.lean`'s `Erases.mkApps` and the spine premise folded per U2.2 (finding **G1-O3** retired); **`green_G1` still elaborates** — the standing obligation, re-measured after U2.5–U2.8; `green_G2`/`green_G3`/`green_G4` elaborate with literal peano answers; `hbridge` has nine fields; `lake exe green-check --all` green; `lake exe hygiene --dup && lake exe hygiene --schedule` exit 0; `test/ledger.expected` re-measured; `doc/coverage.md` updated by hand (the generator is U5.3's) |
...
### W3 — the one simulation, first-order domain, witnesses, the fragment, the pin bump (9 + gate)
...
aggregator takes **those step lemmas as explicit hypotheses**, exactly as W4's U4.1 does, so U3.1's
acceptance does not wait on U3.2/U3.2b/U3.3 and the wave has no cycle (refuter finding B-F6). G3 is
where the hypotheses are instantiated and `erases_correct` first stands alone. **U3.4 is scheduled
first**: two upstream asks are load-bearing this wave (R14, R4), and a refusal must surface on day
one. **U3.8 is scheduled second**: a failed N19 verdict is what would force an η arm back into
`Lower` (R15), and that is cheaper to learn before the ι arm is written than after.

| Unit | Files owned | Depends | Est. | Acceptance |
|---|---|---|---|---|
| **U3.4 upstream-and-repin — first** | the lean4lean **fork** (`Theory/Typing/Injectivity.lean`, `Theory/Typing/InductiveParams.lean`, the seven kernel-generic `CheckerAdequacy` declarations, `WF'.defeqOwn`); `CheckerAdequacy.lean`; **new** `LeanToLambdaBox/Origin.lean`; gate applies the `lake-manifest.json` bump | W2 | 450 | `#check @Lean4Lean.VEnv.IsDefEqU.const_arity_inv` and `#check @Lean4Lean.VEnv.WF'.consts_origin` elaborate at the new pin (**both are load-bearing now** — ask 2 is no longer best-effort, `01-DESIGN.md` §8.3, R4); `Origin.lean` holds and proves the five corollaries this tree consumes — `constOrigin_not_ctorOf`, `constOrigin_not_indInfo`, `IndInfo.inj`, `CtorOf.inj`, `CasesOnShape.inj` — plus the totality direction U2.5's `hclass` needs, each with `#print axioms` recorded; `git show HEAD:lake-manifest.json` names the new rev; `grep -rn "^namespace Lean4Lean" LeanToLambdaBox/` empty (criterion 21); `lake build` green at the new pin; `doc/upstream-asks.md` and `doc/trust.md` rows updated. **If either ask is refused**: land nothing in its place, mark T7 and the ι/proj arms blocked in `doc/trust.md`, and stop the wave for re-scoping — a premise of `IotaRelevant`'s shape is not an option |
| **U3.8 the fragment — second** | `Supported.lean`, `doc/coverage.md` | W2 | 350 | `supportedB` gains the **N19** conjunct (no under-applied constructor or eliminator occurrence) with its soundness lemma into `Supported`; the verdict is computed and recorded for all five VerifyBench programs, and `doc/coverage.md` states per program whether N19 and **N20** hold (N20 by its decidable sufficient condition: every ι-spine prefix argument and minor is already a syntactic value); F-ETA2's row cites the verdict. **If N19 fails on a tracked program**, the unit stops and reports: the contingency (one non-nesting `elimEta` arm plus one collapse lemma, ≈300 lines) is scoped in `04-AMENDMENT-W2.md` §12 and belongs to U2.7's owner, not to this unit |
| **U3.1 the simulation — aggregator and structural arms** | `ErasesCorrect.lean` | W2 **(proof)**, U3.4 (interface) | 950 | `erases_correct` is **stated** exactly as `01-DESIGN.md` §5 prints it — seven binders, six premises, no `hax`, none named `Supported`/`Relevant`/`Iota*`/`*Consistent`/`*Hyps` — and proved with the ι, proj and δ steps as **explicit hypotheses** of the aggregator (`erases_correct_of_steps`), every other case discharged inline: `lam`/`sort`/`forallE`/`ctorVal`/`indVal`/`box`/`app`/β/ζ/`lit`, re-using U2.3's bodies; the `ctorVal` and `indVal` arms are new work — a constructor spine's image is built by `WcbvEval.construct_atom`/`construct_app` and needs the no-over-application fact from U3.4, an inductive-name spine's image is `mkApps .box ts` and folds by `mkApps_box`; `erases_correct_lb` stated over `ErasesLB`; `test/t5.expected` committed and pinned from here on (criterion 6); `#print axioms` in the class-**B** set; `lake build LeanToLambdaBox.ErasesCorrect` green |
| **U3.2 the ι arm** | `ErasesCorrect/Iota.lean` | U3.1 (interface), U3.4 **(proof)**, U2.7, U2.8 **(proof)** | 1,600 | `step_iota` elaborates against U3.1's hypothesis shape, by the six steps of `04-AMENDMENT-W2.md` §6: `erases_mkApps_inv` split; boxed prefix via `erasable_mkApps` + `WcbvEval.mkApps_box`, consuming N20's premises and **adding no premise of its own**; head via the impossibility of the `app`-congruence reading at an `ElimDecl` head (`Lower.const` and `fixConst` both blocked by `¬ RuntimeKey`) plus `CasesOnShape.inj` for the split; discriminant via `not_erasable_of_informative` and `specBlocks`; node facts via `IndBodyOf`/`LowerEnv.inds`; branch via `LowerAlt` + `wcbvEval_mkApps_mkLambdas_substList` + `WcbvEval.lbClosed` + `WcbvEval.iota`; `grep -rn "IotaRelevant\|IotaShape" LeanToLambdaBox/` empty; `not_erasable_of_informative` is a theorem, not a binder; a `_fires` witness; `#print axioms` class-**B**. Priced against the retired 6,823-line ι/proj/data group and `LowerCorrect.lean`'s 2,561 (refuter finding B-F11) |
| **U3.2b the proj arm** | `ErasesCorrect/Proj.lean` | U3.2 **(proof)** | 400 | `step_proj` elaborates, re-using `not_erasable_of_informative` and the `propositional = false` fact and nothing else; its `TrProj` obligation runs through the part of lean4lean's `Verify` layer that is entirely unproven, and the unit's report states exactly which `sorryAx` roots that adds (`doc/trust.md` row handed to the gate); a `_fires` witness on a structure fixture |
| **U3.3 the δ arm, including recursion** | `ErasesCorrect/Delta.lean` | U3.1 (interface), U2.7, U2.8 **(proof)** | 700 | `step_delta` elaborates for both target shapes — `Lower.const` (δ at `Σ` through `LowerEnv.defsTotal` and `wcbvEval_mkApps_head_congr`) and `Lower.fixConst` (`fix_atom` at the empty spine via `Lower.fixBody` + the now-unconditional `LowerBlock.lambda_of_fixLambda`; `fix_guarded` at a non-empty spine via `LowerFix.constToFix`/`Lower.fixUnfold`); the level-instantiation step consumes `ErasesEnv.defns` and is the only place the `instantiateLevelParams` axiom cluster appears (its `doc/trust.md` row handed to the gate); `_fires` witnesses at a recursive and a non-recursive constant |
| **U3.5 first-order** | `FirstOrderInd.lean`; co-owns `Capstone.lean` for the `fo`-parameter swap (finding **G1-O2**) | U3.4 **(proof)**, W2 | 650 | `example : firstOrderIndB arithTable 64 ``Nat = true := by rfl`, likewise `Bool` and `BinaryTrees`' `Tree` (criterion 8 as amended by A6/A15); `firstorder_erases_deterministic` on the eleven-rule `Erases`, where a first-order value's image is unique because a competing `const` derivation's `ConstOrigin` contradicts `CtorOf` through `Origin.lean`'s `constOrigin_not_ctorOf`; `firstorder_no_box` proved of the **lowered** value (finding **G1-O7**), consuming the same `not_erasable_of_informative` as U3.2 — stated once, used twice; `Capstone.lean`'s `{fo : Name → Prop}` instantiated with no reproof of `shipping_erase_correct_firstorder` |
| **U3.6 lowerenv-from-records** | `ColdStartShape.lean` (re-lands `RegInvShape'` adapted), `ErasesEnv.lean`, `SpecEnv.lean` | U2.8 **(proof)** | 750 | `SpecEnv.exists` elaborates from `RegInvShape'` + `ErasureSpec.lookup_adequate` + `htbl`, and discharges `ErasesEnv.defns` from the run's own registry and `ErasesDecl.elim`'s `hsh` from the source table's inductive data; `LowerEnv Σ⁺ s'.gdecls` is **derived** — including `defsTotal` without its `fix` disjunct and the two new clauses `specClosed`/`specBlocks` — not assumed; `EnvAgree`/`WcbvEval.congr_env` elaborate |
...
| **G3 gate — green_G5–G6, the first ι rung, and the closing of T5** | `VerifyBench/Spikes/{G5,G6}.lean`, `Green.lean`, `Capstone.lean`, `ErasesCorrect.lean` (closing instantiation only); N3a files | U3.1–U3.8 **(proof)** | 600 | `erases_correct` is closed: the aggregator's step hypotheses are instantiated with U3.2/U3.2b/U3.3's lemmas and the theorem stands with **no hypothesis beyond its seven binders** — this is where criterion 6 is met and `test/t5.expected` is re-pinned; **`green_G5` — a `match`, hence `.case`, ι and the amended `SEval.iota` — is attempted in the same gate that takes U3.2's arm, not deferred**; `green_G6` (`Nat.add`, hence `_unsafe_rec`, `deltaC`, `.fix`) elaborates with a literal answer; G1–G4 still green; one file constructs a pats-carrying `VEnv.WF` with every ι-round premise, retiring review findings P1 and TA-06 by construction; U3.8's N19/N20 verdicts are reflected in `doc/coverage.md`; criterion-21 grep green; `test/ledger.expected` matches |
...
{U2.6, U2.7} → U2.8 → G2 (only U2.6 and U2.7 are parallel). Inside W3, **U3.4 is scheduled first**
(two load-bearing asks, risks R14/R4) and **U3.8 second** (risk R15: a failed N19 verdict is what
would force an η arm back into `Lower`); U3.1 precedes U3.2/U3.2b/U3.3 by *interface* only, because
its aggregator takes their step lemmas as explicit hypotheses, and **G3** is where they are
instantiated and T5 first stands alone; U3.5 needs U3.4's proof. Inside W4, U4.1 precedes the
...
| W3 | U3.4 | `CheckerAdequacy.lean`'s `namespace Lean4Lean` block (declarations move to the fork at the pin bump) | — | the fork |
| W2 | U2.7 | `LowerCorrect.lean`'s simulation half (`LowerPlain`, `lower_correct_plain`, `lowerFix_correct_plain`, `lower_correct_deltaChain`, `lowerFix_correct_atom`, `LowerNoEta`, `DeltaChain`, `BlockBodiesPlain`, `BlockDefsLambda`, `LowerEnvPlain`), the inversion and spine kit moving to `Lower.lean`; `Lower.{ctorApp, ctorEta, elimEta}`, `CtorDecl`, `ElimHeadOf`, `EtaSpine` | ≈1,900 | T5's one induction (`01-DESIGN.md` §2.3, §5; `04-AMENDMENT-W2.md` §7) |
| W2 | U2.8, U2.9 | `ErasableAxioms`/`AxiomRealizer` and the axiom-realizer table (`Output.lean`), `ErasesDecl.ctor`, `DeltaAgrees` and the four W2 refutation statements in `ErasesCorrect.lean` | ≈300 | the value arms of `SEval` (A18) and `ErasesDecl.defn` (A22) |
| W3 | U3.1 | `ElimBody.lean`'s evaluation theory (`mkElimBody_iota_fwd`/`_bwd`, `wcbvEval_mkLambdas_fwd`, `mkElimBodyRec`) | ≈600 | nothing evaluates at `Σ⁺` (`01-DESIGN.md` §2.3) |
