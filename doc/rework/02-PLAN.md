...
### W2 — β, ζ, literals, projections, then the re-anchoring (4 + 6 + gate)
...
**U3.4 is scheduled first**: two upstream asks are load-bearing this wave (R14, R4), and a refusal
must surface on day one. **U3.8 is scheduled second**: a failed N19 verdict is what would force an
...
where they are instantiated and T5 first stands alone; U3.5 needs U3.4's proof and **U3.2's**
(`not_erasable_of_informative`, stated once and used twice), so W3's serial path is
U3.4 → U3.8 → U3.1 → U3.2 → {U3.2b, U3.5} → G3. Inside W4, U4.1 precedes the
...
rows below are restated accordingly; the tool defect itself is a `doc/dev-fix-queue.md` row, since
...
| W3 | U3.4 | `CheckerAdequacy.lean` — partial: the `namespace Lean4Lean` block only, its declarations moving to the fork at the pin bump | — | the fork |
