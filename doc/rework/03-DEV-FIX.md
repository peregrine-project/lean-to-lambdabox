...
programs it emits, against the committed expectations. The *commit* column carries the hash
and the subject; a commit cannot record its own hash, so the hashes were written in by the
review follow-up that closed the branch — for the last two rows, each by the commit after it,
and for the final row by the merge.

The last two rows, **F-DEPLCTX** and **F-ARITYLET**, are W7's findings, raised in
`dev/verify`'s copy of this file; their *Reported, not fixed* sections below are marked
*Fixed*, as the other nine findings' are.

| id | commit | files and functions | behaviour before | behaviour after | emitted bytes | test | verification obligations |
|---|---|---|---|---|---|---|---|
| F-SPARSE | `dd2e2ee` `fix(F-SPARSE)` | `LeanToLambdaBox/Erasure.lean`: `visitCases`; new `LBTerm.hasLooseBVarFrom`/`LBTerm.hasLooseBVar` | A sparse `casesOn` panicked at the `unreachable!`, erased the whole elimination to `.box`, exited 0 and wrote a program `peregrine validate` accepts and `peregrine eval` either mis-evaluates or gets stuck on ("`Case: <15> branch not found`") | The inductive is read from `casesInfo.indName` and the catch-all is expanded into one alternative per uncovered constructor, in constructor order; the shapes that remain uncompilable (side-condition elimination, machine-`Nat`/`Int` discriminee, alternatives that do not match the constructors, a catch-all with a free index) `throwError`. No `unreachable!` and no path to a wrong `.ast` with exit 0. Follow-up (review F-review.md §3.1): the catch-all's hypotheses were boxed without checking they are proofs — `Meta.isProof` under a `Meta.lambdaBoundedTelescope` on the catch-all's own arguments, the criterion `firstNonProofField` already uses for a constructor's fields, `throwError`s naming the offending hypothesis's position otherwise; the `logInfo` announcing the expansion, new stdout on every tracked program that reaches it, is removed rather than kept and marked | yes — `Quicksort` only, **+231 bytes**, this fix's share of the branch's cumulative Quicksort delta measured in one driver run against both builds (review F-review.md §3.5): baseline 65826 → the fully-fixed branch's 66409 (+583 total), of which F-ETA accounts for 352 (11 registered fixpoints × 32 bytes, F-ETA's own row) and this fix for the rest — the catch-all body is emitted once per uncovered constructor, once here. The row's earlier figure (65474 → 65686) was measured in a different driver environment from F-ETA's row (66025 → 66377) for the same program with no intervening change, so the two are not mutually comparable, which is why they are replaced with a single consistent pair; `Arith`, `Sieve`, `BinaryTrees`, `Fannkuch` and rungs G1–G6 remain byte-identical, and the new hypothesis check changes no byte count anywhere it does not refuse | `test/fixes/F-SPARSE.lean` | `visitCases`'s body changed, so the `VisitExprRefines` step bodies that mirror it must be re-proved (no mutual member added or removed: the `partial_fixpoint` arity is unchanged). The fragment is unaffected — `Supported.supportedHead` still refuses `isSparseCasesOn`/`isMatcherName` heads, and `CasesOnShape` (`SourceEval.lean:163`) and `BlockAdequate.casesOnDecl` (`ErasureSpec.lean:202`) may keep `c.getPrefix = I`, which holds of every head they admit. Covering the expanded shape would need `indName` there plus an `Erases`/`Lower` arm for a catch-all alternative and an `ErasesCorrect/Iota.lean` case. **Closed** (M4, M7): `CasesInfoAgreesK.indName`/`.altCtor` (`ErasureSpec.lean`) supply `altIdx`'s totality, `altCtor` combined with `ReifiedInduct.Pinned`'s per-constructor `cidx` for injectivity (P12); `visitCasesBody` is re-copied verbatim and `step_visitCases` re-proved against the rebuilt loop, with the five F-SPARSE refusals and F-ACC's stepped to their tests and killed by `run_throwError_ne_ok` |
| F-UNSAFEREC | `e88a265` `fix(F-UNSAFEREC)` | `LeanToLambdaBox/Erasure.lean`: `visitMutual`; `LeanToLambdaBox/Basic.lean`: `ModPath`/`Kername` gain `deriving DecidableEq` (`List.Nodup`'s `Decidable` instance needs `DecidableEq` on the element type) | `remove_unsafe_rec` strips one literal `_unsafe_rec` component and is not injective: a `mutual` block legally declaring both `u` and `u._unsafe_rec` mapped to `[u, u]`, so two `FixDef`s were named `u` and two declarations were registered at the one λbox key `u` — `fixvarMap`'s second binding silently overwrote the first, no error, exit 0 | Once the block names are mapped through `remove_unsafe_rec`, `visitMutual` `throwError`s unless the mapped kernames are pairwise distinct (`List.Nodup`), before the `withReader`/registration that would otherwise collide | no — every tracked block (`VerifyBench`'s five programs and rungs G1–G6) is a singleton (`scratch/round7/C-fixes.md` §10(3): 63 blocks, 63 singletons), on which a one-element list's `Nodup` holds unconditionally, so the guard cannot fire; not re-measured by rebuilding `VerifyBench` here, which sits outside this task's shipping-closure build scope | `test/fixes/F-UNSAFEREC.lean` | `EraserAsks.block_keys_distinct` (`ErasureSpec.lean:385-395`) states an *unconditional* fact about `getDeclInfo?`'s mapped keys; the guard does not make that true, it makes the *run* fail on a violating block, so retiring the field means restating distinctness as a conclusion of a successful run and rewiring its consumers (`VisitExprRefines/Step/Env.lean`, `ErasureRun.lean`, `ColdStartRun.lean`, `Bridge.lean`'s `BlockKeyed.nms.Nodup` conjunct) rather than deleting it outright. **Closed** (M2, M4, M7): `EraserAsks.block_keys_distinct` is deleted; `ErasureRun.run_rec_exit_nodup` steps the `unless` guard and hands `blockKeyed_install` the `Nodup` fact directly, in the field's place, through `run_visitMutual_registers` |
| F-KERNAME | `4e354a7` `fix(F-KERNAME)` | `LeanToLambdaBox/Erasure.lean`: new `checkKernameFresh`, called from `addAxiom` and from the two `toKername`-minting registration points in `visitMutual` (the nonrecursive single-declaration branch and the mutual-fixpoint registration loop); new `ErasureState.indBlocks` and `checkIndKernameFresh`, called from `register_inductive` | `toKername` collapses `.num p k`/`.str p k.repr` to one key, and `cleanIdent`'s escape has fixed points, so two distinct Lean names can mint the same λbox key; the second `gdecls.cons`/`constants.insert` silently overwrote the first entry, no error, exit 0, and the printer emitted whichever declaration was registered last | `checkKernameFresh` scans the already-registered `constants` for an entry at the same kername under a different name and `throwError`s naming both, before the insert that would otherwise collide; the key encoding itself (`toKername`, `cleanIdent`) is unchanged. Follow-up (review F-review.md §3.2): `register_inductive` mints a *mutual block's* key the same way, off `rootKername (String.join (indinfo.all.map toString))` — a bare concatenation, so `[AB, C]` and `[A, BC]` mint one key exactly as `.num`/`.str` do for a single name — and that registration point was unguarded: neither checked against a same-key constant nor against a different block already at that key. `ErasureState.indBlocks` now records each registered block's key alongside its member list, and `checkIndKernameFresh` — called right after `register_inductive` computes the block's key, before any of the block's constructors are processed — `throwError`s if that key is already a different block's, or already a constant's; `checkKernameFresh` gained the matching check in the other direction (a constant key that is already a block's). Neither direction changes the key encoding | no — the refuse variant changes no bytes on any input that does not collide; the finding's own census over the whole elaboration environment (228,987 constants, 228,987 distinct keys, 0 collisions) found no live instance, so no tracked program exercises the new check. The block-collision guard is exercised only via `test/fixes/F-KERNAME.lean`'s own direct call, with the review's own `[AB, C]`/`[A, BC]` witness (`String.join` gives `"ABC"` both ways, confirmed by `#eval`) — no tracked program's inductive blocks collide either | `test/fixes/F-KERNAME.lean` | `Supported.kernameSepB`/`SupportError.kernameCollision` (`Supported.lean:241-245`) and `Bridge.BlockKeyed`'s fourth conjunct (`Bridge.lean:164-171`) already state kername separation as a restriction over the tabled names rather than assume it holds generally; the guard changes nothing about that — it makes the *run* enforce the restriction the fragment already states, rather than leaving a silent collision unchecked. `toKername_not_injective` (`VisitExprRefines/Step/Env.lean:740`) stays true and is not touched. **Closed** (M2): `run_checkKernameFresh_ok`/`run_checkIndKernameFresh_ok` step both guards, restoring every broken `rw [run_modify]` and handing `RegInvShape'.addAxiom`'s `hfresh` premise a run-derived half; `registerIndState` writes `indBlocks` alongside `gdecls` |
| F-DEPTH | `33ba53e` `fix(F-DEPTH)` | `LeanToLambdaBox/Relevance.lean`: `isArityCheck` | The loop's fuel was `ty.approxDepth.toNat + 1`, an 8-bit field that saturates at 256 whatever the *reduced* telescope's real depth, and is 0 (fuel 1) at any definitional alias whatever telescope it unfolds to; a ≥ 256-binder telescope or an `@[irreducible]` alias hiding one threw before reaching the closing `.sort`, so `Erasure.isErasable` fell back to `isErasableMeta`, which does not unfold such an alias either and answered `false` — an inductive **type former** judged relevant | The fuel is a fixed constant (100000), independent of the unreduced subject's syntactic depth, so both shapes decide inside the kernel instead of throwing; `isArityCheck.loop` and its call sites are unchanged, and the single `isArityCheck.loop <fuel> ty` call shape is preserved (the constant-fuel variant, not iterative deepening, per C-refute C3) | no — none of the five tracked programs' type formers reach an `@[irreducible]` alias or a ≥ 256-binder telescope (`Nat`/`Bool`/`List`/`Option`/`Prod`/`Tree`-shaped, depth ≤ 4); not re-measured by rebuilding `VerifyBench` here, which sits outside this task's shipping-closure build scope | `test/fixes/F-DEPTH.lean` | `EraserAsks.kernel_ind_head_true` (`ErasureSpec.lean:374-384`) names this row as its refutation (a telescope of ≥ 256 binders or an alias); the fix removes both counterexamples this round demonstrated but does not retire the field by itself — its statement still needs restating (C-refute C2/C4: e.g. as the disjunction "the kernel run answers `true` or raises the fuel error", not a side condition that only excludes the counterexample) since no syntactic bound on the unreduced subject bounds the reduced telescope. `isArityCheck.loop.WF`/`isArityCheck.WF` (`RelevanceCheck.lean:104-137`) are untouched — proved `∀ fuel`, so they hold at the new constant with no proof change. **Closed as a restatement, not a proof** (M4): `EraserAsks.kernel_ind_head_true` stays a field with its statement unchanged; its docstring is corrected to name the residue a constant fuel cannot bound — a *reduced* telescope longer than the budget, and any other kernel error inside `isArityCheck` — rather than the two counterexamples this row already measures gone |
| F-ETA2 | `c911803` `fix(F-ETA2)` | `LeanToLambdaBox/Erasure.lean`: `visitCtorEtaGo`, `visitCasesEtaGo`; new `etaArgIsValue` and `withEtaPrefixLets` (a CPS helper outside the mutual block) with its `@[partial_fixpoint_monotone]` lemma `withEtaPrefixLets_mono` | Both loops pushed the fresh variables onto the argument array and erased the whole application at the bottom, so the arguments the call site had already supplied were emitted **under** the binders the loop opened: `λ x⃗. C ⟦a⃗⟧ x⃗`, which weak evaluation re-runs on every application where the source runs it once. Measured at HEAD: `@List.cons Nat (slowSum 12)` erased to `λ. ((C₁ □) (slowSum 12)) #0` and `Nat.casesOn _ (slowSum 12) 0` to `λ. case (slowSum 12) […]` | Each supplied argument that is neither already a variable nor erased to `□` (both are values, so placing them under the binders duplicates nothing) is erased once, in the outer context, and bound by a `let` outside the binders; the loop then opens its binders with that variable in the argument's place — `let a₁ := ⟦a₁⟧; … λ x⃗. C a₁ … x⃗`. Only the outermost round binds anything: every argument the recursion adds is a variable. In the eliminator loop the candidates start at `casesInfo.discrPos`, since `visitCases` reads the major premise and the alternatives and drops what precedes them — binding a dropped index would evaluate an argument the emitted program does not. Measured: `let(slowSum 12). λ. ((C₁ □) #1) #0` and `let(slowSum 12). let(0). λ. case #2 […]`, both `peregrine validate`-clean and evaluating to the same values as before. The deletion the section proposes is *not* taken (C-refute: a short constructor block is ill-formed and stuck after `constructors_as_blocks`), and no mutual member was added or removed. Follow-up (review F-review.md §2, the review's one substantive finding): `visitCasesEtaGo`'s loop bound every supplied argument from `casesInfo.discrPos` onward — the discriminee *and* every already-supplied alternative — but an alternative is branch-guarded in the `case` node `visitCases` emits, reached only under its own constructor, so hoisting one into an unconditional `let` forces it on every application, including the branch not taken: measured, a diverging zero alternative on a discriminee that always selects the succ branch went from evaluating (pre-fix and under Lean's own semantics) to no value at any fuel. Narrowed to `a.2 == casesInfo.discrPos` (only the discriminee), per the review's own diff: the eliminator loop now binds one variable, not one-plus-however-many-alternatives were already supplied, and the alternative's own erasure happens exactly where `visitCases` already puts it — inside its branch, once eta-expansion finishes supplying the missing arguments. `visitCtorEtaGo` is unchanged: a constructor's fields are unconditional in the emitted block, so hoisting all of them stays strictness-neutral | no wherever no occurrence is under-applied — the new code sits in the `args.size < arity` branch alone, and the four other `test/fixes` programs (`pick`, `quicksort`, `plain`, `even`) re-emit byte-identical `.ast` across the edit; `doc/coverage.md` reports `SupportError.underAppliedCtor`/`.underAppliedElim` 0 times over thirteen tables, so no tracked program reaches the branch. Where it does fire, the term gains one `tLetIn` per bound argument (`expanded_tLetIn`, `EEtaExpandedFix.v:38`, preserves expandedness, so the consumer's η conditions are unaffected). The narrowing changes `test/fixes/F-ETA2.lean`'s own `elim` spine from `[let, let, λ, case]` to `[let, λ, case]` (one `let`, for the discriminee only) and adds a third program, `divergeTest`, the review's diverging-alternative reproducer: `elimDiverge`'s zero alternative is `partial def spinF n := spinF (n+1)`, never selected (the discriminee is `3`), which the narrowed fix leaves unevaluated — `peregrine eval` gives `5`, where the pre-narrowing shape gave no value at any fuel (checked directly: reverting to the un-narrowed loop and re-erasing `divergeTest` reproduces exactly that) | `test/fixes/F-ETA2.lean` | The fix does **not** lift N19: a `let`-prefixed η-expansion still has no `Lower` arm (the `ctorEta`/`elimEta` arms were deleted at A21), so this is a shipping-soundness fix and `Supported.lean:70-77`/`doc/coverage.md` keep the restriction — but `doc/coverage.md`'s N19 paragraph must drop the claim that the constructor half disappears once this row is repaired, which C-refute refutes. The *bodies* of `visitCtorEtaGo`/`visitCasesEtaGo` changed while their signatures did not, so the mirroring `Stepᵢ` bodies in `VisitExprRefines/Step/Mechanical.lean` and `Step/Passes.lean` must be re-proved; the motive count stays 18, since `withEtaPrefixLets` is a helper beside the block and not a member of it, leaving the `partial_fixpoint` arity unchanged. **Closed** (M7): `visitCtorEtaGoBody`/`visitCasesEtaGoBody` are re-copied from the shipping bodies and `step_visitCtorEtaGo`/`step_visitCasesEtaGo` stay one-liners on the saturated path, the new code sitting in the `else` branch N19 already makes dead; `doc/coverage.md`'s N19 paragraph and this file's own two wrong sentences above are corrected (§2.7 of `doc/rework/10-MERGE-FIXES.md`) |
| F-ACC | `f42d0aa` `fix(F-ACC)` | `LeanToLambdaBox/Erasure.lean`: `visitCases`; new `arityResultSort`, `isPropositionalArity` and `firstNonProofField` (helpers beside the mutual block) | An elimination of a propositional inductive whose constructor binds a non-proof field was erased to a `.case` binding that field. Lean admits large elimination for such a `Prop` when the field is recovered from the result indices, and `Acc.casesOn` does bind `Acc.intro`'s `x : α`; the downstream collapse (`remove_match_on_box`, `EOptimizePropDiscr.v:48`, and `eval_iota_sing`, `EWcbvEval.v:162`) substitutes `□` for **every** binder of the surviving alternative, so it boxes that data. Today the collapse never fires — every emitted inductive is declared non-propositional (F-PROP) — and the program is merely stuck; the moment the flag is set the same program computes a wrong value under no error | `visitCases` `throwError`s before emitting, naming the inductive, the constructor and the field's index. The criterion is the shape, not the name: `isPropositionalArity` is MetaRocq's own recipe (`destArity` on the declared arity, then `Sort.is_propositional`, `Extract.v:276`) and the fields are classified by `Meta.isProof` under the `forallBoundedTelescope` `register_inductive` already opens, so an `Acc` clone is refused as `Acc` is. `And`, `Iff`, `Eq`, `Or` and `False` are still erased: their fields are all proofs, which the collapse boxes soundly. Follow-up (review F-review.md §2's neighbouring question): `visitProj`, the symmetric site turning a source `Expr.proj` into λ□'s own `.proj`, needs no matching guard, checked rather than assumed — Lean's structure-projection elaborator already refuses to generate a projector for a non-proof field of a `Prop`-valued structure ("field must be a proof, but it has type Nat" on `structure PropData : Prop where n : Nat`), and the kernel's own `.proj` typing rule independently refuses one built by hand: `fun (h : Nonempty Nat) => Expr.proj \`Nonempty 0 h` (`Nonempty.intro`'s field is data, not a proof — the same shape as `Acc`'s `x`, minus the index that makes `Acc.casesOn`'s case legal) constructed directly and run through `Lean.addDecl` reports "(kernel) invalid projection". So no already-typechecked term `visitProj` is ever handed can name such a field; `.proj` reaching a `Prop`-valued structure at all only happens through `register_inductive`'s own `is_struct` path, whose fields are the union type's, never a bare `Expr.proj`-on-data-in-Prop | no — the edit adds a refusal and no emission path; the finding's own measure (`grep -c 'Acc\|WellFounded\|Quot' VerifyBench/ast/*.ast` = 0 on all five) says no tracked program reaches it, and the six `.ast` the other `test/fixes` programs emit (`pick`, `quicksort`, `plain`, `even`, `cons`, `elim`) are byte-identical across the edit, measured by re-emitting them against the stashed pre-fix build | `test/fixes/F-ACC.lean` | The restriction `Supported.propElimIntoData` (`Supported.lean:65-69`, checked at `supportedHead:321`/`supportedGo:355` via `informativeB`) is **not** lifted — the guard makes the shipping code enforce a *subset* of it (C-refute C11: `propElimIntoData` refuses every non-informative inductive, the guard only one with a non-proof field; `Acc` is in both), so `doc/coverage.md`'s row stands. Nothing in `Erases`, `Lower`, `SEval` or the capstone changes. The *body* of `visitCases` changed while its signature did not, so the mirroring `VisitExprRefines` step body must be re-proved; no mutual member was added or removed, since the three helpers sit beside the block, leaving the `partial_fixpoint` arity unchanged. The full treatment — substituting the recursor's own index argument for an index-determined field, i.e. a Lean-specific `iota_sing_idx` in `SEval`/`Erases`/`Lower`/`erases_correct` — stays blocked on lean4lean's `Injectivity.lean` sorries. **Closed** (M7): `step_visitCases` steps the refusal as a loop over `firstNonProofField`'s `for` (P11's correction to the plan's "two branches" undercount), through `run_firstNonProofField_okW`, never assumed away |
| F-PROP | `d28459f` `fix(F-PROP)` | `LeanToLambdaBox/Erasure.lean`: `register_inductive`; `LeanToLambdaBox/Basic.lean`: `OneInductiveBody.propositional` loses its default | `OneInductiveBody.propositional` was never supplied, so the `false` default — carrying the author's own hedge — was emitted for every inductive, `Prop` ones included (71/71 `false` over the five programs). `erases_mutual_inductive_body` (`Extract.v:276`) states the field as an *equality* with `isPropositionalArity` of the source arity, so `false` is as wrong as `true` in the other direction; and since `remove_match_on_box` (`EOptimizePropDiscr.v:48`) and `eval_iota_sing` (`EWcbvEval.v:162`) are the only rules that eliminate a boxed discriminee, an elimination of a `Prop` into data shipped **stuck**: `peregrine eval` answers `Case: <15> branch not found` on the `And` and `Eq` programs of `test/fixes/F-PROP.lean` | `register_inductive` sets the field by MetaRocq's own recipe — `arityResultSort` is `destArity` (syntactic, no reduction) and `isPropositionalArity` is `Sort.is_propositional` on the sort it ends in, taken as `Level.isAlwaysZero` so that a `Prop` reached through `max`/`imax` counts. The default is removed, so the field can no longer be forgotten. `kelim` is untouched (C-refute C9: `ind_kelim` is carried through `EReorderCstrs`/`EUnboxing` and printed, but no MetaRocq or peregrine pass reads it to decide anything), and its docstring now says so. Measured: the `And` and `Eq` programs evaluate to 5 and 3 where they were stuck, under a `.ast` peregrine `validate` accepts as before | yes, one byte per propositional inductive and only there — `one_inductive_body "And" false` → `… true` (2371 → 2370 bytes on `prop-and.ast`, 2970 → 2969 on `prop-eq.ast`). The 23 inductive names the five benchmark programs emit (`Add … Tree`) are all data, so their flag stays `false`; measured directly on the six `.ast` the other `test/fixes` programs emit (`pick`, `quicksort`, `plain`, `even`, `cons`, `elim`) plus `data.ast`, all byte-identical across the edit | `test/fixes/F-PROP.lean` | The flag is read by `isPropositionalInductive` (`Semantics/Env.lean:20-27`) and thence by `WcbvEval.iota`/`iota_block`/`proj` (`= false` premises), `iota_sing`/`proj_prop` (`= true`) and `LBOptimize` (`Optimize.lean:41,60`). Inside the fragment nothing changes: `Supported.informativeB` is `resultSort … |>.isNeverZero`, which no inductive satisfying the new flag can satisfy, so every inductive an elimination site admits keeps `propositional = false`. That argument covers eliminations and projections only, not inductives registered from a *constructor* occurrence (C-refute C10), for which the missing step is that a `Prop`'s constructor occurrence is erasable and never reaches `visitConstructor` — a fact about the relevance oracle. Five comments asserting "the erasure marks no inductive propositional" (`Semantics/Flags.lean:24-30`, `ElimBody.lean:87-89`, `Erasability.lean:280`, `Supported.lean:66`, `:562`) become false as stated and must be restated over the fragment or as a per-rung measurement. `Supported.propElimIntoData` is **not** lifted: that needs the singleton-elimination theory (F-ACC's full treatment), not the flag. `register_inductive`'s body changed, so the `VisitExprRefines` step body mirroring it must be re-proved, and every construction of `OneInductiveBody` in the verification must now supply the field. **Closed** (M1, M4, M5): `PropositionalInd`/`propositional_false_of_informative` (`Erasability.lean`) state the equation MetaRocq's `erases_mutual_inductive_body` carries and derive `= false` where `InformativeInd` is in hand, rather than assuming it of every registered body; `IndBodyOf` keeps its λ□-only arity (P7b) and the derived conjunct moves to `ElimDecl`; the five comments are restated over the fragment |
| F-QUOT | `046cbc4` `fix(F-QUOT)` | `LeanToLambdaBox/Erasure.lean`: the `ci.value? = .none` arm of `visitMutual`; new `addRealizer`, `mkAnonLambdas` and `quotRealizer` (helpers beside the mutual block) | Lean's quotient primitives have no value, so all four took the body-less-axiom arm. A program using `Quot.mk`/`Quot.lift` erased to one that applies two axioms: `peregrine validate` accepts it and `peregrine eval` refuses to compile it — `Axioms found, use Extract Constant to realize them in C: .Quot.lift, .Quot.mk` | The arm keys on `ConstantInfo.quotInfo` and registers the realizer `quotRealizer` gives its `QuotKind`, at the arity the kernel fixes: `Quot.mk` (3) is `λ _ _ x. x`, `Quot.lift` (6) is `λ _ _ _ f _ q. f q`, and `Quot`/`Quot.ind` — a type former and a proof, both erased at every use site — get `□`. `Quot.sound` is an `axiomInfo`, not a `quotInfo` (C-refute), so it is untouched and stays body-less; it is proof-valued, so no use site reaches it. Anything else with no value still takes the axiom arm. Measured: `test/fixes/F-QUOT.lean` goes from two body-less constants and a refused compile to none and `6` | yes, on any program reaching a quotient primitive: two `(constant_body None)` become bodies. None of the five benchmark programs or the other `test/fixes` programs reaches one — `grep -c Quot` is 0 on all five (`F-QUOT`'s own row in this file), and the suite is byte-identical across the edit | `test/fixes/F-QUOT.lean` | `Supported.quotPrim` (`Supported.lean:55`, checked at `supportedHead:313`) is **not** lifted: the fragment still refuses a quotient primitive in a computationally relevant position, and lifting it needs `SEval`/`Erases`/`Lower` arms for `Quot.lift`'s β-like reduction and quotient constants in lean4lean's model. What does change is the *run*: `visitMutual`'s `.none` arm gains a fourth registering exit, so `run_visitMutual_decomp`'s three-disjunct decomposition (`ColdStartRun.lean:376-386`), the `ax` field of the cold-start induction principles (`ColdStartInduction.lean:280-284`, `:114`) and `RegInvShape'.addAxiom` (`ColdStartShape.lean:215`) need an `addRealizer` case — `addRealizerState` registers `(toKername n, .constantDecl ⟨some t⟩)` where `addAxiomState` registers `⟨none⟩`, so the shape lemmas carry over with `DefnDecl` in place of the body-less entry. `visitMutual`'s body changed, so the `VisitExprRefines` step body mirroring it must be re-proved. **Closed** (M2, M6, M7): `addRealizerState`/`run_addRealizer_ok` sit beside `addAxiomState`/`run_addAxiom_ok`; `run_visitMutual_decomp`'s three disjuncts become five and `RegInvShape'.addRealizer` is `addAxiom`'s twin at a `DefnDecl` premise; the two new exits are *proved*, not refuted from the fragment (§3.2, correcting P15's own miswording of M7's brief) — `run_visitMutual_registers` takes no `Supported` premise, so `addRealizer` is discharged exactly as `addAxiom` is |
| F-EQREC | `ed692a6` `fix(F-EQREC)` | `LeanToLambdaBox/Erasure.lean`: the same `ci.value? = .none` arm of `visitMutual`; new `recursorRealizer` (beside `register_inductive`, which it calls) | A recursor has no compiler value, so every recursor reached as a constant was emitted body-less and then applied. `Fannkuch` ships one, `((MPdot (MPfile ()) "Eq") "rec")`; `peregrine validate` accepts the file and `peregrine eval` refuses to compile it — `Axioms found, use Extract Constant to realize them in C: .Eq.rec` — so the benchmarks only ran because they hand peregrine an `eq_rec_c.attr`/`eq_rec_ml.attr` the frontend does not emit | The arm synthesizes the body MetaRocq gets for free (Rocq's `eq_rect` is an ordinary constant whose body is a `match` on a propositional singleton, which `remove_match_on_box` collapses): at the recursor's calling convention — parameters, motives, minors, indices, major premise — `λ…. case (I, nparams) (bvar 0) [alt]`, the alternative binding the constructor's kept fields and handing the single minor every field, an erased one as `□`. The shape is read off the `RecursorVal` and the inductive it names, never off the constant's name: a single non-recursive `Prop` (F-PROP's `isPropositionalArity`) with at most one constructor, all of whose fields are proofs (F-ACC's `firstNonProofField`). `Eq.rec` gets `λ _ _ _ _ _ _. case (Eq,2) (bvar 0) [([], bvar 2)]`, which agrees with the benchmarks' hand-written `fun _ _ _ x _ _ => x`; `And.rec` gets the same shape with two field binders and `bvar 3 (bvar 1) (bvar 0)`, which the collapse substitutes `□` for; `False.rec` a `case` with no alternative. Every other recursor — `Nat.rec`, `Acc.rec`, `Or.rec`, `Exists.rec` — keeps today's body-less emission. Measured: `test/fixes/F-EQREC.lean`'s three programs go from one body-less constant each and a refused compile to none and `9`, `4`, `2`; `runBenchmark 5` of `VerifyBench/Src/Fannkuch.lean`, erased closed, goes from the same refusal to `7`, which is what Lean computes, with no `.attr` | yes — `Fannkuch` only (38653 → 38968 bytes: it gains the `Eq` inductive block and `Eq.rec`'s body and loses its one `(constant_body None)`). `Sieve` is byte-identical, and `Arith`, `BinaryTrees`, `Quicksort` and rungs G1–G8 emit no body-less constant at all (`grep -c '(constant_body None)'` = 0), so the arm never fires on them; the other `test/fixes` programs' expectations are unchanged | `test/fixes/F-EQREC.lean` | `NoBodylessRefs` — the capstone's own decidable premise, false on `Fannkuch` (`doc/coverage.md:60-66`) — becomes true there. `SupportError.recursorHead` (N21) is **not** lifted: `Supported.supportedHead`'s `isRecursorName` arm (`Supported.lean:331`) rejects a recursor head because recursors are tabled body-less, so a program reaching one is vacuously covered; lifting it needs an `SEval` ι rule for Lean recursors, an `Erases`/`Lower` arm at the synthesized body and an `erases_correct` case (`doc/rework/probes/Q2-subsingleton.md` §2's L7 plus `SubsingletonElim`), which inherits lean4lean's `VEnv.WF'.consts_origin` ask. As with F-QUOT, `visitMutual`'s `.none` arm now has a further registering exit, so `run_visitMutual_decomp` (`ColdStartRun.lean:376-386`), the `ax` field of the cold-start induction principles and `RegInvShape'.addAxiom` need the `addRealizer` case; the body registered there is a `.case` over an inductive `register_inductive` has just registered, so the registry invariant sees the two writes in that order. Follow-up (review F-review.md's tracked note): `recursorRealizer`'s docstring showed the `And.rec` example's two field binders as a bare fact, disconnected from the `remove_irrel_constr_args` paragraph two lines below it that says proof fields "leave the alternative's binders" — both are true (the example is the `:= false` default; the erased shape only appears under `:= true`), but read side by side as contradictory. The docstring now derives the example from the argmask directly, states which config it assumes, and gives `And.rec`'s shape under the other config alongside it (`case (And,2) (bvar 0) [([], bvar 1 □ □)]`) instead of only describing it in prose. No code changed. **Closed** (M2, M6, M7): the same registering-exit machinery as F-QUOT's row — `NoBodylessRefs` becomes true on `Fannkuch` (§2.8's strict gain, recorded in `doc/coverage.md`'s census column), and `SupportError.recursorHead` is not lifted: `supportedHead`'s refusal of every quotient primitive and recursor head is what keeps a realizer key off a supported program's reachable set, measured rather than assumed (P13) |
| F-ETA | `42989c5` `fix(F-ETA)` | `LeanToLambdaBox/Erasure.lean`: new `etaExpandFix` (beside `mkDef`), called from `visitMutual`'s mutual-fixpoint registration loop; the eraser's own `-- TODO: eta-expand fixpoints?` above that loop is answered and removed | The body registered for a recursive declaration was the bare `.fix defs i`. It evaluates — λ□ treats an unapplied fixpoint as a value and unfolds it on application — but it falsifies `EEtaExpandedFix.expanded_tFix` (`EEtaExpandedFix.v:46-54`), which admits a `.fix` only under an argument spine longer than the selected member's `rarg`, and so `expanded_eprogram_env`, the declared `pre` of `guarded_to_unguarded_fix` (`ETransform.v:666-672`) and thereby of peregrine's whole verified untyped pipeline. Nothing downstream reported it: peregrine's own discharge of that precondition is `Admitted` (`Transforms.v:375`) and `validate` runs `EWellformed.all_env_flags`, which does not look | The registration point wraps the fixpoint the way MetaRocq's `eta_fixpoint` (`template-rocq/theories/EtaExpand.v:72`) does: `fun x₀ … xₙ => (fix defs i) x₀ … xₙ` with `n = defs[i].principalArgIdx`, one binder at the `principalArgIdx = 0` every `mkDef` emits and `rarg + 1` in general. The clause's other two conjuncts hold as they stand: every member body is λ-headed, and all 56 self-references across the five programs carry at least one argument (re-measured on the regenerated post-fix programs with `scratch/round7/eta_selfrefs.py`: 0 under-applied). Evaluation is unchanged — both shapes are values and agree on every application | yes — every program that registers a fixpoint. Measured by regenerating the five corpus programs from one driver against both builds: `Arith` 14013 → 14141, `BinaryTrees` 29288 → 29608, `Fannkuch` 39600 → 40080, `Quicksort` 66025 → 66377, `Sieve` 27762 → 28082, i.e. exactly 32 bytes per registered fixpoint (4/10/15/11/10) and nothing else — `peregrine eval --anf=false` gives byte-identical output on all five before and after, and all five still `validate`. The committed `VerifyBench/ast/*.ast` and the eight rung environments in `Green.lean` are verification artifacts and are *not* regenerated on this branch | `test/fixes/F-ETA.lean`, which carries the `expanded` fixpoint clauses itself — `peregrine validate` has no η check | `LowerEnv.defs` (`ErasesEnv.lean:229-232`) must gain the wrapped declaration shape, and its one destructuring consumer is the δ step (`ErasesCorrect/Delta.lean:172-182`), whose witness *is* the emitted body: `Lower Γspec b₀ (λ. (fix …) 0)` is an η arm of `Lower`, and `Lower`'s η arms were deleted deliberately (`01-DESIGN.md:437-444`, A21), so this needs either a bounded η arm at the declaration shape or a δ step restated to absorb one β before `fix_guarded` (C-refute C1) — a new simulation case, not a restatement. Also `LowerFix.lean`, the nine `LowerEnv` reconstructions in `ColdStartShape.lean` (`:237, :247, :309, :319, :437, :439, :507, :510, :535`) and the `visitMutual` mirrors in `VisitExprRefines/Step/Env.lean`, `ErasureRun.lean`, `ColdStartRun.lean`, `ColdStartInduction.lean`, `Bridge.lean`, `Lower.lean`, `Witness/SourceTable.lean`. `LBWfPeregrine.fixLambda` is untouched — it constrains member bodies, not the wrapper. The emitted constant's *value* is now `λx. (fix …) x` where the `Lower` image of the spec constant's value is `.fix defs j`; the capstone survives that only because its observable is restricted to first-order answers. What is gained: with `Green`'s eight rungs regenerated and an `LBExpandedFix` decision procedure beside `lbWfPeregrine_of_check`, the capstone can conclude `PeregrinePre` (`Output.lean:265-267`) instead of only `LBWfPeregrine`. **Closed** (M1, M3, M5, M6, M8): `Lower.fixEta`, the η-expanded-fixpoint arm, relates a block member's specification body to `LBTerm.etaFix defs j` beside `fixBody`'s bare node; `LowerEnv.defs` gains the matching disjunct and `ErasesCorrect/Delta.lean`'s δ step moves one line to `fixEta_of_block`, with the extra β absorbed in `Lower.appReady`'s new case. `LBWfPeregrine` itself is strengthened — `expandedFix` folds in all three missing conjuncts of MetaRocq's `expanded_tFix` as one clause — so `PeregrinePre` is not a separate conclusion any more, it is **deleted**: what the sentence above called "the stronger predicate" is now what `LBWfPeregrine` states, and every rung's `hwf` is decided against it by the same `by decide +kernel` (`doc/rework/10-MERGE-FIXES.md` §4.7) |
| F-DEPLCTX | `341e2a9` `fix(F-DEPLCTX)` | `LeanToLambdaBox/Erasure.lean`: the two `withReader` re-entries of `visitMutual` — the non-recursive exit and the block's sibling loop | Both re-entries moved the level column (and, at the non-recursive exit, the fixvar map) and left `ErasureContext.lctx` in place, so a dependency's body — a *closed* term — was erased under whatever local context the caller's term walk had open when it reached the `.const` node, and every relevance verdict taken below ran lean4lean's checker at the pair `(lctx := the caller's local context, lparams := the dependency's column)`, whose declarations may mention level parameters that column does not have. MetaRocq has no such step: `erase_constant_body` (`../metarocq/erasure/theories/Extract.v:264`) erases `cst_body cb` in the **empty** context at `cst_universes cb`, because the global environment is erased by a pass of its own rather than from inside the term traversal | Both re-entries reset `lctx := {}` alongside the level column, so a dependency is erased in the context a closed body has and the pair handed to the oracle is the one `erase_constant_body` erases at. `fixvars` is *not* reset in the sibling loop — it is the block's own map, installed by the `withReader` that loop runs under, and it is what the member's self- and sibling-references resolve through; at the non-recursive exit it keeps its existing `.none`, since a non-recursive declaration is not a member of the caller's block. Behaviour-preserving by construction: `ci.value!` is closed, so no `fvar` of the caller's context occurs in it, and the `Meta` calls made below (`inferType`, `isErasable`, `Meta.isProof`, the `forallBoundedTelescope`/`lambdaBoundedTelescope` calls) answer the same at a closed subject in either context. Where a body were *not* closed the reset turns a silent mis-scoped erasure into a loud "unknown free variable", which no measured program produces | no — measured, not argued: 21 programs erased from a standalone driver (`LeanToLambdaBox.Erasure` only, `csimp := false` as the `VerifyBench/*.lean` roots set it, output to a scratch directory rather than the committed `VerifyBench/ast/`) against both builds. The five corpus programs — `Arith` 14141, `BinaryTrees` 29580, `Fannkuch` 40132, `Quicksort` 66459, `Sieve` 28125 bytes — and the 16 `.ast` the ten `test/fixes` tests emit are **byte-identical** across the edit, and so is every one of the 21 runs' stdout, i.e. the eraser's own `logInfo` stream (`@[extern]`, inline and realizer decisions) down to the line | `test/fixes/F-DEPLCTX.lean` | This is the gate on `Motive6`'s content (`U5-report.md` §5, `U7-report.md` §5, `U9-report.md`): with the pair well scoped, `MLCtx.WF env Us` is satisfiable at the dependency's column — `TrExprS env Us` of every entry's type (`lean4lean/Lean4Lean/Verify/TypeChecker/Basic.lean:158`) no longer has to hold of binders typed at parameters the column drops — so `BridgeInv` becomes rebuildable at a member sub-run, read at the `Δ = []` that `ErasesEnv.defns` and `erase_constant_body` state a registered body's erasure at. `U5.mlctx_not_wf_at_empty` and `U5.bridgeInv_nil_lctx` (`scratch/round7/u5_lctx.lean`) are the two facts that made the old reader's shape unusable there; they stay true of the old shape and stop applying to the new one. What that unblocks, in the order the reports give: `RegInvShape'.defs`'s `Lower Γspec b₀ t` at a registration, `blockKeyed_install`, `run_rec_exit_nodup`/`run_rec_exit_reg`'s fifth conclusion, `run_mkFreshFVarId_list` and `run_rec_exit_siblings_chained` — all written for this consumer and unconsumed until now. The bodies of `visitMutual`'s two re-entry points changed while its signature did not, so the `VisitExprRefines` step body mirroring it must be re-proved, and every `run_visitMutual_*` decomposition that reconstructs the reader at a sub-run (`ColdStartRun.lean`, `ColdStartInduction.lean`, `ErasureRun.lean`) now reconstructs it at `lctx := {}`. The test passes on **both** sides of the edit, measured by rebuilding the pre-fix `Erasure.lean` and re-running it: there is no shipping-observable difference to catch — a closed body cannot see the caller's context — so it is a guard on the invariant, not a reproduction of a miscompile |
| F-ARITYLET | `b3db162` `fix(F-ARITYLET)` | `LeanToLambdaBox/Erasure.lean`: `arityResultSort`, read by `isPropositionalArity` and hence by `register_inductive`'s `propositional` field (F-PROP), by `recursorRealizer`'s shape test (F-EQREC) and by `visitCases`' refusal (F-ACC) | The walk read `.forallE` and `.sort` and answered `none` at everything else, where MetaRocq's `destArity` (`../metarocq/pcuic/theories/PCUICAst.v:486-490`) walks `tLetIn` as well. A declared arity may carry a `let` — `inductive FooLet : (let _x := Nat; Prop)` elaborates and `InductiveVal.type` keeps the `letE` — and `.mdata` sits in the same position (`Lean.addDecl` accepts an inductive whose type is `.mdata … (.sort .zero)`). At those shapes a `Prop` was emitted non-propositional, so its elimination shipped **stuck**, which is precisely the failure F-PROP repaired everywhere else: measured against the pre-fix build on this row's own test, `FooLet`, `FooParam` and `FooMData` all flagged `false`, and `peregrine eval` answered `Case: <15> branch not found` on both emitted programs | Two arms, `.letE _ _ _ b _ => arityResultSort b` and `.mdata _ b => arityResultSort b`. The first *is* `destArity`'s `tLetIn` arm; the second is Lean's annotation node, which every kernel walk sees through. A `Level` never mentions a term variable, so returning one from under the `let`'s binder is sound exactly as it already was under `.forallE`'s. The walk deliberately still does **not** reduce, against this task's brief: `Lean.whnf` would also see through `def MyArity := Prop`, `inductive FooAlias : MyArity` (whose `InductiveVal.type` is `.const MyArity []` — measured, it elaborates), but lean4lean's `TrExprS` is *not* transparent at `.const`, so `vResultSort` of the translated type is `none` there and a `true` flag would falsify `ErasureSpec.propositionalInd_of_arity` (`ErasureSpec.lean:554`) — the direction of MetaRocq's equation that is proved and that `propositional_false_of_informative` spends. `.letE` and `.mdata` are precisely the arms where `TrExprS` *is* transparent (`../lean4lean/Lean4Lean/Verify/Typing/Expr.lean:164-170`), which is why they can be walked and `.const` cannot. Measured after: the three flags are `true`, the two programs evaluate to 5 and 4, and `FooAlias` stays `false` | no on anything tracked — the five corpus programs are byte-identical across the edit and so are the 16 `.ast` the other `test/fixes` tests emit (whole suite green, driver as in the F-DEPLCTX row); the corpus census is unmoved at 71 `false` + 1 `true` (`Fannkuch`'s `Eq`), i.e. no corpus inductive's arity carries a `let` or an `mdata`. Where it does fire it is F-PROP's one byte, the other way: `one_inductive_body "FooLet" false` → `… true`, `let-prop.ast` 2208 → 2207 and `param-prop.ast` 2106 → 2105 | `test/fixes/F-ARITYLET.lean` | `IndFlagSound` (`ErasesEnv.lean:66`) does **not** become the biconditional, and the row should not be read as claiming it: the converse stays refuted by two shapes this edit measured rather than removed. (i) `inductive FooBVar : (let u := Prop; u)` elaborates with `InductiveVal.type = .letE … (.bvar 0)`; `TrExprS.letE` substitutes the let value, so the image is `.sort 0` and `PropositionalInd` holds, while the walk answers `none` at the `.bvar` — and so does PCUIC's `destArity`, whose `tRel` arm is `None` too, so this is a gap between lean4lean's zeta-reducing translation and `destArity`, not a gap between this function and `destArity`. (ii) the alias arity above, where closing it would be unsound as argued. What the edit does move: `vResultSort_of_arityResultSort` (`ErasureSpec.lean:518`) gains two cases — its catch-all `simp [Erasure.arityResultSort] at har` no longer closes `.letE`/`.mdata` — and each is the body's own induction hypothesis at the `TrExprS` arm that is transparent there, so `propositionalInd_of_arity` extends to let- and mdata-carrying arities with no new hypothesis and `IndFlagSound` stops being vacuous at them. That proof update is owed at the merge. `Supported.resultSort` (`Supported.lean:211`) is a second copy of this walk and was **not** touched here (it is verification code): it still stops at `.forallE`, so `informativeB` answers `false` at a let-carrying arity, which keeps F-PROP's argument — every inductive an elimination site admits keeps `propositional = false` — true as stated, at the cost of the fragment refusing a let-carrying *data* inductive as non-informative. Giving it the same two arms is a merge-side edit. Downstream of the flag, `recursorRealizer` and `visitCases`' F-ACC refusal now see let- and mdata-carrying `Prop`s as propositional, which is the point: both were written against `isPropositionalArity` and neither names a shape |

## Reported, not fixed

Every `*Site.*` line number below is the line the finding was *raised* at, kept so the
finding reads as it was written; the shipping edits moved them (`register_inductive`
`Erasure.lean:192-241` → `:295`, `visitCases` `:770`/`:768-835` → `:1000`, the
`ci.value? = .none` arm `:873-876` → `:1165`, `visitMutual` `:859-919`/`:906` → `:1151`,
`isArityCheck` `Relevance.lean:46` → `:49`, `OneInductiveBody.propositional`
`Basic.lean:164` → `:166`). The named declarations, which `Tools/Hygiene.lean` checks, are
unchanged.
...
*Fixed* on `dev/fix` by `fix(F-PROP)` (`d28459f`), after `fix(F-ACC)` — see the shipping-edits table.
...
*Fixed* on `dev/fix` by `fix(F-ARITYLET)` (`b3db162`) — see the shipping-edits table. The walk
gained `destArity`'s `tLetIn` arm and Lean's annotation arm, and no more: it still does not
reduce, because `TrExprS` is transparent at `.letE` and `.mdata` and opaque at `.const`, so a
reducing walk would flag an alias-headed arity `true` where `vResultSort` of the translated type
is `none`. The equation therefore still holds in **one direction only**, and for a residue the
edit measured rather than removed: `inductive FooBVar : (let u := Prop; u)` elaborates with a
`.bvar`-headed body, where `TrExprS.letE` substitutes the let value and `PropositionalInd` holds
while the walk answers `none` — as PCUIC's `destArity` does at `tRel`, so the gap is between
...
*Status in the verification, at the time this row was raised.* The capstone's conclusion
stated `LBWfPeregrine`, which deliberately did **not** claim fixpoint η; the stronger
`PeregrinePre` was defined and *not* concluded, and the difference was exactly this row.

*FIXED* by `fix(F-ETA)` (`42989c5`): `visitMutual` registers `etaExpandFix defs i`. See the Applied
edits row for the measured byte change and the obligations it moves.

*Verification status now* (M1, M3, M5, M6, M8): `LBWfPeregrine` is itself strengthened —
its `expandedFix` clause carries the conjunct `PeregrinePre` used to state separately — so
`PeregrinePre` is **deleted** rather than concluded alongside it, and the capstone's
`LBWfPeregrine Γ t` conjunct is already the stronger statement. `Lower.fixEta` is the new
relation arm the η-expanded body needs; `doc/rework/10-MERGE-FIXES.md` §4 is the design in
full.
...
*Fixed, first half only,* on `dev/fix` by `fix(F-ETA2)` (`c911803`) — see the shipping-edits table. Both
...
paragraph carried the same claim and is now corrected to match (M8, C-refute C8). N19 itself
stands: a `let`-prefixed η-expansion has no `Lower` arm either.
...
*Fixed* on `dev/fix` by `fix(F-SPARSE)` (`dd2e2ee`) — see the shipping-edits table. The inductive comes
...
*Fixed* on `dev/fix` by `fix(F-ACC)` (`f42d0aa`), before `fix(F-PROP)` — see the shipping-edits table.
...
*F-QUOT fixed* on `dev/fix` by `fix(F-QUOT)` (`046cbc4`) — see the shipping-edits table. The arm keys on
...
*F-EQREC fixed* on `dev/fix` by `fix(F-EQREC)` (`ed692a6`) — see the shipping-edits table. The recursor
...
then throws, `Erasure.isErasable` (`LeanToLambdaBox/Erasure.lean:177`, now `:201`) takes its `.error` arm,
...
*Fixed* on `dev/fix` by `fix(F-DEPTH)` (`33ba53e`) — see the shipping-edits table. The constant-fuel
...
*Fixed* on `dev/fix` by `fix(F-UNSAFEREC)` (`e88a265`) — see the shipping-edits table. The guard sits exactly
...
*Fixed* on `dev/fix` by `fix(F-KERNAME)` (`4e354a7`) — see the shipping-edits table. The refuse variant:
...
*Fixed* on `dev/fix` by `fix(F-DEPLCTX)` (`341e2a9`) — see the shipping-edits table. Both
re-entries reset `lctx := {}` beside the level column, which is the pair `erase_constant_body`
erases at; `fixvars` is unchanged at either site. Nothing shipping-observable moves — a closed
body cannot see the caller's context — so the test is a guard on the invariant, not a
reproduction of a miscompile. `U5.mlctx_not_wf_at_empty` and `U5.bridgeInv_nil_lctx` stay true
of the old reader's shape and stop applying to the new one, so `BridgeInv` becomes rebuildable
at a member sub-run at `Δ = []`, and the `run_visitMutual_*` decompositions reconstruct the
sub-run's reader at the closed-body reader.
