...
review follow-up that closed the branch — and, for the last two rows, each by the commit after
it, which leaves the final row carrying its subject alone.

The last two rows, **F-DEPLCTX** and **F-ARITYLET**, are W7's findings. They were raised in
`dev/verify`'s copy of this file, which has moved on since this branch was cut, so their
*Reported, not fixed* sections are not below; marking them *Fixed* there is part of the merge.
