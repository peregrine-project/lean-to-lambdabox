# The `dev/fix` queue — shipping defects raised by the verification

The repository's standing rule is *raise implementation issues, do not silently patch them*.
Each row below is a defect in `LeanToLambdaBox/{Erasure,Basic}.lean` that the verification
found, specified with the site, the command that measures it, and that command's output. No
wave depends on any of them landing, and no unit applies one: they belong to branch
`dev/fix`, whose index of *applied* edits is `doc/rework/03-DEV-FIX.md` (today: `F-FUEL`
only, and that one is verification-authored code, not shipping code).
