...
entry point. `green_G8` (W5) is still
...
stand on `shipping_erase_correct_firstorder` and on the six rungs, because
...
| `lookup_adequate` | a run's constant and inductive lookups agree with the specification environment. Three clauses are guarded or two-sided: `declInfo` puts the visited name in the block `Lean.Compiler.LCNF.getDeclInfo?` answers with, under the guard `Lean.Compiler.isUnsafeRecName? n = none` — the primitive prefers the `_unsafe_rec` twin, so the membership is false at such a name; `ctorArity` answers exactly for the constructors `lenv` declares and for no other name; `casesInfo` answers exactly for the `casesOn` constants, at metadata agreeing with the declared block through `CasesInfoAgreesK` | — |
...
| `block_adequate` | the kernel's inductive blocks, constructors and eliminators are the model's, at the identifier `Erasure.register_inductive` mints: `fwd` and `bwd` between `IndInfo` and a declared block with its `KernelFields`, `ctor` and `ctorBwd` between `CtorOf` and a declared `ConstructorVal`, and `casesOnDecl` for the eliminator's `CasesOnShape` | — |
...
| `hprep` | the prepared term is the rung's own subject — `Erasure.prepare_erasure` is the identity at it. Load-bearing because the capstone's syntactic conjuncts read the prepared term while `Supported` and `TrExprS` are checked at the subject, and transporting those two across the passes is a stronger statement about `Lean.Compiler.LCNF.macroInline`, false in general | `lake exe reify --prepared` runs `Erasure.prepare_erasure` against the live environment and reports identity; green at all six rung subjects |
| `hdih` | `DeclInfoAtHead`: at a `KnownHead` name `Lean.Compiler.LCNF.getDeclInfo?` answers `some`, and the name is not an `_unsafe_rec` twin. The two facts `lookup_adequate.declInfo` is short of — it has no `r = none` arm and no supplier for its guard | the same `lenv` read the clause is; the honest end state is two more clauses of `declInfo` |
| `hctab` | a name the table holds in its declaration column that `lenv` declares a constructor is in the table's constructor column too. A `lenv` read of the table's own columns, like `TableSafe`; measured true of 24 of 24 constructor entries over the nine committed tables | `lake exe reify --check`, once it is a `SourceTableAdequate` clause. It holds by construction of the reifier, whose `.ctorInfo` arm also reifies the constructor's inductive block |
| `hall` | a declared inductive is a member of its own block, `iv.name ∈ iv.all`. The one clause `block_adequate` is short of; `step_visitConstructor` takes it inline | the kernel's own block invariant; the honest end state is a `BlockAdequate` field |
| `hseg` | the elaborator's `Lean.CasesInfo` has one alternative per constructor of the tabled block, and its `discrPos` is the model's discriminant position. The two clauses `CasesInfoAgrees` is short of; `step_visitCases` takes them inline | `lake exe reify --check` for the first, once it is a `CasesInfoAgrees` field derived in `CasesInfoAgrees.of_pinned`; the second is a `CasesInfoAgreesK` or `BlockAdequate` clause |
...
|---|---|---|
| `hbridge` | the six results the run's final state carries beyond the erasure itself, together with the existence of the specification environment they are read at: `erasesEnv` (`SpecEnv.erasesEnv`), `lowerEnv` (`RegInvShape'.lowerEnv`), `wfSpec` (`ErasesEnv.keys` for the `Nodup` half, `SpecEnv` for the `ClosedBodies` half), `wf` (`visitExpr_shape_all` for the term half of `closed` and `ctorApplied`, W5 for the environment halves and `fixLambda`), `simulate` (`simulate_of_erases_correct`, W3, proved) and `firstorder` (`firstorder_erases_deterministic` and `firstorder_no_box`, W3, proved). The last two are stronger than the theorems named by the premises a clause cannot supply at an applied subject. All six wait on the registration invariant at the final state — `RegInvShape'`, `RegSaturated` — which is W5's, and W5's supplier also pays `SpecEnv.fvarFree` with `Erases.noFVar` | nowhere, at any rung. The erasure half is no longer in the bundle: `erasure_bridge_of_run` proves `Erases env [] [] pe t₀ ∧ Lower Γspec t₀ t` from the run, supplying seventeen of the eighteen member steps |
| the bridge's residual step obligations | two, plus the four class-**D** rows above (`hdih`, `hctab`, `hall`, `hseg`). `Step4`, the obligation at `Erasure.visitConst`, has **no supplier**: `Motive4` classifies the head by `KnownHead` and excludes neither a constructor nor a type former, and at a nullary tabled constructor the run emits a `.const` that no rule of `Erases ⨟ Lower` relates to it; `step4_of_exclusions` is the same statement with those two exclusions as premises, and it is proved. `VisitExprRunConcl` is a second, unconditional induction over the same eighteen-member family, which the tree does not have — `visitExpr_shape` propagates a state predicate and neither `Erasure.RunConcl` nor the generator bound is one | nowhere. Each is a binder of `erasure_bridge_of_run` and hence of `shipping_erase_correct_firstorder` and of every rung. Owner: this repository; `Motive4`'s two exclusions and the unconditional induction are both W5's |
