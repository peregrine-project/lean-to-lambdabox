...
| `block_adequate` | the kernel's inductive blocks, constructors and eliminators are the model's, at the identifier `Erasure.register_inductive` mints: `fwd` and `bwd` between `IndInfo` and a declared block with its `KernelFields`, `ctor` and `ctorBwd` between `CtorOf` and a declared `ConstructorVal`, `casesOnDecl` for the eliminator's `CasesOnShape` **at the block's own segmentation** `iv.numParams + 1 + iv.numIndices`, which is what pins the model's discriminant position to the elaborator's, `selfMem`, a declared inductive is a member of its own block (`iv.name ∈ iv.all`), which is what the registration loop of `Erasure.register_inductive` is indexed by, and, since round 7 wave 4 (W6), two more: `selfName`, a declared inductive is declared under its own name (`lenv.find? I = some (.inductInfo iv) → iv.name = I`) — without it the member loop's `.inductInfo` match at `Erasure.lean:346` could cons a block entry with no member actually registered — and `fields`, a declared inductive's constructor records are the kernel's own, at their positions (`∃ nfs, KernelFields lenv iv nfs`) — `fwd`'s fourth premise, stated of `lenv` directly because the only other producer, `Witness.SourceTableAdequate.inds`, answers for tabled names alone. The emitted propositional flag is **not** a ninth clause: `ErasureSpec.propositionalInd_of_arity` *derives* the half the consumers spend — a `true` `Erasure.isPropositionalArity` verdict gives `PropositionalInd env I` — from `decl_adequate` and the arity walk's commutation with translation, and the converse is false at an arity whose final sort sits under a `let`, at the let's own bound variable (`doc/rework/03-DEV-FIX.md`, F-ARITYLET; the refutation itself is now a proved theorem, `arity_of_propositionalInd_false`, not an argued claim — round 7 wave 4, W9), so assuming the equation here would assume something refuted. What that costs is stated where the half is used: `IndFlagSound` (`ErasesEnv.lean`) fires only at a `true` flag, and no body any rung emits carries one, so the clause is **vacuous at all eight rungs** — the `= false` the ι and projection arms read is derived at the consumer from `InformativeInd`, not read off the environment relation, and F-ARITYLET's shape (flag `false`, model propositional) is harmless only because `SEval.iota`'s `hinf` then has no derivation. `propositionalInd_of_arity` itself has no consumer in the tree yet — nothing constructs an `ErasureSpec` to call it at — so the derivation is available but unspent | — |
...
| `oracle_refl` (reflection clause) | the monadic oracle run reflects the kernel predicate, at the scope the verdict was taken under: the clause fires under `ctx.lparams = Us` and the bridge takes the bundle at **every** `Us`, so a run below `Erasure.visitMutual`'s `withReader (… lparams := ci.levelParams)` spends it at the member's own column. Its second disjunct is where the `Erasure.isErasableMeta` fallback lives, assumed sound. The separate `oracle_meta` clause — a `true` verdict at a scope *other* than the reader's — is gone with U5: with `P` at every scope, `ctx.lparams` is always a scope `oracle_refl` covers, and a clause conditioned on `ctx.lparams ≠ Us` had no reachable call site | near-definitional; the discharge's price is (a3). The fallback arm is empirically dead on the error route: 0 fallback hits in 139,196 constants |
...
| `SchemeNames` (round 7 wave 5, W9-A) | `lenv` declares its quotient primitives and its recursors under the two name tests `Supported.lean` defines: `quot`, a `.quotInfo` name is one of the four `Erasure.quotRealizer` dispatches on (`quotPrimNames`); `recr`, a `.recInfo` name carries `Supported.recSuffix` (`I.rec`, an eliminator generated beside it, or an auxiliary recursor `I.rec_k`). Taken beside `P : ∀ Us, ErasureSpec lenv env Us gw` rather than inside it, since neither field mentions a level scope, and landed in `Supported.lean` rather than `ErasureSpec.lean` to avoid an import cycle. Not yet taken by any rung: it first enters a rung's hypothesis list at `doc/rework/12-REPAIRS-W9.md` §2.5's `visitExpr_regInv_all` (W9-D), not landed. Spent by `no_realizer_exit`/`no_realizer_exit_compiler` to exclude F-QUOT's/F-EQREC's two realizer-registering exits from the fragment (`doc/rework/07-STATUS.md` §4) | measured true at 230,479 constants of this toolchain (`scratch/round7/w9a_measure.out`): the `.quotInfo` constants are exactly `quotPrimNames` (`Quot.sound` is an `.axiomInfo`, not a `.quotInfo`), and every `.recInfo` constant carries a `recSuffix`. No `reify` verb reads either field; both are class-**D** for the same reason `block_adequate` is — no term denotes `lenv` |
...
`hnb` is no longer in this table either. `NoBodylessRefs Γ t` was never a hypothesis
`shipping_erase_correct_firstorder`'s proof spent (`scratch/round7/z_dead.out`), so
`doc/rework/11-REPAIRS-W8.md` §2.8 deletes the binder from the theorem and from all eight
rungs' applications of it rather than carry an unread premise. `Green.g<i>_noBodylessRefs`
stays a standalone `by decide +kernel` term at each rung — **0 failures**, Fannkuch's
pre-fix body-less `Eq.rec` now being a `.case` body of `recursorRealizer`'s registering
(F-QUOT/F-EQREC) — and `doc/coverage.md`'s per-rung census (`RungFacts.nbTerm`) is its only
remaining reader; that column reads `env.find?` alone, an existence census rather than a check
of the term's content (recorded in `Tools/Coverage.lean`'s own prose, round 7 wave 5, W9-H).
...
|---|---|---|
| `hbridge` | the two environment results the run's final state carries beyond the erasure itself, together with the existence of the specification environment they are read at: `erasesEnv : ErasesEnv env bo lp Γspec t₀` and `lowerEnv : LowerEnv Γspec Γ`. The binder's whole payload is composed by `bridgeEnv_of_regContent` (`LeanToLambdaBox/Capstone.lean`) out of four facts about the run's final state — `RegAcc` (round 7 wave 4, W5: `RegInvShape'` with its content clause `RegContent` and the saturation clause `SpecKeysEmitted` folded into one accumulator), `RegKeyed`, and `ErasuresDeclared` at the prepared term — and no other premise: `hdeps`, the dependency closure `bridgeEnv_of_regInv` takes, is discharged from the accumulator's own δ column by `ReachableFrom.isSome_of_declaredEnv`, and the binder's `Lower Γspec t₀ t` premise is not spent. Wave 3's finding that the composition's premise as it then stood, `hsub` (declaration equality between the specification and the emitted entry), was jointly `False` with `RegContent`'s content clause and with the eliminator entry at every rung is **repaired**, not merely renamed: W1 restates it as `SpecKeysEmitted`, reading only the emitted entry's *shape*, proved satisfiable and non-vacuous at the rungs it used to refute (`scratch/round7/W4-refute.md` §2). What still has no producer for a run of the shipping eraser is `RegAcc` itself: `RegKeyed` **is** now produced at a run, `regKeyed_of_run` (round 7 wave 4, W6, by induction over `Erasure.visitExpr`), but `bridgeEnv_of_regContent` is never called at that value or at any other — it has zero consumers anywhere in the tree, and is not on `shipping_erase_correct_firstorder`'s own proof path. `RegContent`'s `defns` field at a fresh block registration is the standing gap: `regInv_registerInd_step` (`ColdStartShape.lean`) takes the block's prefix content as a premise rather than deriving it, because `IndCovered` — what every member of a freshly registered block needs — is never *introduced* anywhere in the tree (round 7 wave 4, W5, re-planned as `doc/rework/12-REPAIRS-W9.md`'s W9-B/W9-C/W9-D; round 7 wave 5's W9-B attempted the producer and returned blocked — round 7 wave 6 landed its five-unit decomposition, B-i…B-v, closing the gap: `regInv_registerInd_run` (`VisitExprRefines/Step/Passes.lean`) now proves `RegAcc` preserved across **one** block registration, and the same wave's W9-C proves the recursive-constant sibling, `regInv_recConst_step`. Neither is called by `bridgeEnv_of_regContent`, which still has zero consumers: what is still missing is `StepAcc6`, the accumulator step that would chain either theorem across a run of `Erasure.visitExpr` as a whole (round 7 wave 7's `W9-D1`/`W9-D2` land seventeen of the eighteen `StepAccᵢ` obligations the chain needs, `StepAcc6` not among them — `doc/rework/07-STATUS.md` §4, `scratch/round7/B-v-report.md`, `scratch/round7/W9-C-report.md`, `scratch/round7/W9-D2-report.md`). `ErasuresDeclared` is refuted at a block member's sub-run. **The dependency's local context** (F-DEPLCTX, `doc/rework/03-DEV-FIX.md`) is **closed**, round 7 wave 4: `Erasure.visitMutual`'s two dependency re-entries now reset `ErasureContext.lctx` alongside the level column, so `MLCtx.WF` has a witness at a member sub-run, `BridgeInv` is rebuildable there, and W4 spends exactly that to prove `bridgeInv_member`/`visitMutual_member_erases`/`_block` — themselves zero-consumer declarations, since the registration invariant they would feed (`RegAcc` at a run) is still not produced, for the unrelated reason above. **α**: `ReifiedDecl.Prepared` pins a tabled body only up to `Expr.AlphaEq` — deliberately, since on equality the clause is uninhabited wherever `inlineMatchers` fires, and `lake exe reify --check` reports five of G7/G8's bodies matching only up to binder names — while `Lower` is not α-closed on its source, its two `fix` arms reading a declared body out of `Γ` by equality. **The ∀-`Γspec` shape**: `RunRefines` reads content at *every* `SpecEnv` of the final state, where the repair produces one; wave 4's plan to fix this by restating `RunRefines` with an independent fifth conjunct is superseded (round 7 wave 5, `12-REPAIRS-W9.md` §2.5, W9-D): the accumulator instead goes in its own second bundle of eighteen motives, `AccGrows`, proved by a second `Erasure.visitExpr.mutual_fixpoint_induct` instance that reads the first bundle's conclusion as a hypothesis at one step only, so `RunRefines` and its eighteen existing step lemmas stay untouched. Not landed; depends on W9-B (above) and W9-C, also not landed. Neither the α gap nor this shape gap moved this round (round 7 wave 5 also measured the tabled-`casesOn` dependency, W9-H, and excluded the two `quotRealizer`/`recursorRealizer` exits from the fragment by name, W9-A, neither of which touches `hbridge`'s own composition — `doc/rework/07-STATUS.md` §1/§4) | nowhere, at any rung. These two fields are all the bundle carries. the well-formedness field left it and is the capstone's own binder `hwf`, a checked term at every rung; the simulation field is retired, the capstone applying `erases_correct` once at the spine with `ErasesEnv.mkApps` and a per-argument premise; the box-freedom field is retired, box-freedom of the lowered value coming from `noBox_lower_of_foSpine` on the constructor-tree shape `firstorder_erases_core` now exports; and the answer's uniqueness is the capstone's own `firstorder_erases_core` call. The erasure half is `erasure_bridge_of_run`, which proves `Erases env [] [] pe t₀ ∧ Lower Γspec t₀ t` from the run and supplies all eighteen member steps |
...
| `hbody` (named `hargReach` before round 7 wave 4, W7/U9) | at **G8 only**: **every** erasure of `benchArith`'s tabled body reaches `Nat`'s block, quantified over every specification environment `Γspec` rather than read at the run's own one — `∀ Γspec b, g8Table.body? ``benchArith = some b → ∀ b₀, Erases env (g8Table.levels? ``benchArith) [] b b₀ → ReachableFrom Γspec b₀ natIid.mutualBlockName`. It is what remains of the observable clause's per-argument `ErasesEnv` conjunct after `Green.g8_argErasesEnv` reduces that conjunct at the argument's image `peanoLB 0` to a reachability fact (`Green.g8_hargReach`, `Green.lean:1528-1541`), and W7 makes `green_G8` take that residue directly rather than the run's own `ErasesEnv`/`Lower` antecedents the old `hargReach` needed. Strictly **stronger** than the binder it replaced, and the strengthening is forced: `Γspec` is not in scope at the binder any more, so the strongest available instance is the `Γspec = []` one, equivalent by `reachable_nil_iff` to a direct-occurrence claim on `constRefs`. Not vacuous and not refutable: the tabled body is `fun n => 2 ^ (((n * 3) - n) + 3)`, whose `OfNat.ofNat` literals erase through `Erases.lit` to peano towers that do name `Nat`'s block (`scratch/round7/W4-refute.md` §1.2) | nowhere. The content clause is an *antecedent* of `Green.g8_hargReach`, the lemma the binder now composes with, not a thing it waits on: `ErasesEnv.defns` at the tabled body of `benchArith` produces the declared erasure `b₀`, the lowering premise refutes the `box` and `ctor` arms of `Erases` at the subject, and `ReachableFrom.through_body` reduces that lemma to the one residue `hbody` states directly. That residue is a fact about `Erases` at a reified body, a non-erasability argument on the source side, not about the registration invariant. G1-G7 apply the observable clause at the empty spine, where the premise is vacuous |
...
| `UpstreamAsks env` | the remaining load-bearing upstream asks (`doc/upstream-asks.md` items 2, 9 and 10), as four fields. `constsOrigin` — a constructor belongs to one type at one index, a type former has one λ□ identifier and one arity, an eliminator's segmentation is fixed, a type former declared below `env` is declared by `env`'s own list, and two blocks below `env` declaring one former are the same block. `constOriginExcludes` — a plain constant is neither a constructor nor an inductive type name — is **refuted**: `test/Vacuity.lean`'s `not_upstreamAsks_natEnv` derives `False` from it at the fixture environment, because a block's constants are each declarable as an axiom of a well-formed list below `env` (`Origin.lean`'s `CtorOf.constOrigin`). **This structure is inhabited at no environment declaring an inductive block — and every rung binds it together with one of three things a block always supplies (`FirstOrderInd`, `SpikeNatFacts`, or a tabled block through `P`/`htbl`), so the accurate statement is stronger than "vacuous there": no environment satisfies a rung's hypotheses at all** (`scratch/round7/W7-refute.md` §1.2, mechanised as `Rungs.green_G5_absurd`/`green_G6_absurd`/`green_G6_absurd_table`; the capstone's own observable conjunct is likewise unreachable under its own `A`, guarded as it is by `FirstOrderInd env I` under the conclusion's `∀ I`). The conclusion the refuted premise guards is itself false at the one environment this tree can build: `Fixture.no_unique_erasure_natZ` gives `Nat.zero` two distinct erasures at `natEnv`, refuting the rungs' own uniqueness conjunct — `UpstreamAsks` is not merely an unused premise, it is the premise whose falsity keeps a false conclusion from being reachable. Ask 2's classification conjunct has left the structure — it is `consts_classified`, off `constOrigin_of_wf` — and so has `CtorOf → ¬ IndInfo`, which `CtorOf.not_indInfo` proves. `mkAppsInv` — spine typing inversion, stated with `OrderedStrong` explicit — and `indSpineInj` — two defeq spines headed by inductively declared formers have the same head. Item 6 (`constArityInv`) is no longer a field: it landed at the pin, `sorry` (`Injectivity.lean:45`), and its three consumers — `not_erasable_of_informative`, `indSpine_ne_forallE` and `FirstOrderInd.notSortNotPi` — cite `Lean4Lean.VEnv.IsDefEqU.const_arity_inv` directly, so item 6's obligation is now an inherited `sorryAx` root rather than a hypothesis this row carries (the ledger's two direct rows for these, `not_erasable_of_informative` and `fOFields_of_asks`, already measured `sorryAx` before the swap, through `sort_inv`/`forallE_inv_stratified`/`sort_forallE_inv`, so no ledger row's footprint changed). Taken by `erases_correct` (hence T5 and everything closed on it), `not_erasable_of_informative`, `indSpine_not_prop`, `elim_major`, `ctor_saturated`, `CasesOnShape.agree`, `fOFields_of_asks`, `firstorder_erases_deterministic` and `firstorder_no_box`, and unpacked by `LeanToLambdaBox/Origin.lean`'s corollaries. `erasure_bridge_of_run` takes it too, for the bridge's steps 3, 4 and 17, so every rung now carries it | for `mkAppsInv` and `indSpineInj`, the pin bump: the fork's `HasType.mkApps_inv` and `IsDefEqU.indSpine_inj` build them with no change to any consumer's statement shape. Not for the two item-2 fields: `VEnv.WF'.consts_origin` has landed and pays the classification conjunct only. Their discharge is a downstream restatement — the five readings must be read off `env`'s **own** declaration list, as MetaRocq's `declared_constant` reads `lookup_env`, after which `consts_origin` gives all of them (`doc/upstream-asks.md` item 2). Item 6 has already landed this way, its consumers citing the fork directly rather than through this structure. Not measurable in the ledger — `#print axioms` reports a proved theorem's footprint, never a hypothesis — so this row is where it lives |
...
| `hfo` | `FirstOrderInd env I`: the answer's inductive is first order in the model — every constructor field of every member of its block is itself a first-order inductive, and the block has no parameter, index or `Prop` member. Read at `` `Nat`` by all eight rungs | nowhere. `firstOrderIndB` decides the same property on a reified `SourceTable` and `firstOrderIndB_step` is its table-side half, but the model-side half is the kind transfer from `lenv` to `env.constants` — `doc/upstream-asks.md` item 4, the same ask `ErasesEnv.tabled`'s exclusion waits on — so no rung discharges it by computation. Round 7 wave 7's `HFO` unit narrowed item 4 to a concrete missing fact rather than proving it: the round-4-landed `TrEnv.inductInfo_inv` gives the block's member list but no declaration list, so it cannot be identified with the block `IndDeclOf` exhibits; the fusion that would (`TrEnv'.inductInfo_wf'`) is stated but not landed upstream, and a second gap (`lenv.find?` vs `lenv.toKernelEnv.find?`) is a specification-bundle decision beyond it (`scratch/round7/HFO-report.md`, `doc/rework/07-STATUS.md` §4). Independent of both, discharging `hfo` from `P`/`htbl` is now self-defeating: `indInfo_of_tabled` alone already contradicts `A` by the same route `UpstreamAsks env`'s row above names, so no rung can hold `hfo` and `A` together regardless |
