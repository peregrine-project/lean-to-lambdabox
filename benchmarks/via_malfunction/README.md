...
- `PEREGRINE_NO_UNBOX_SWITCH`: in which the `peregrine` tool,
  modified to accept "raw lambdabox", is installed.
  The modified `peregrine` source code is available at https://github.com/simon-dima/lambda-box-extraction/tree/release.
  This switch is used when the Make variable `UNBOX` is set to `0` (e.g. when using the backend `malfunction-prune-nounbox`).
- `PEREGRINE_UNBOX_SWITCH`: in which the `peregrine` tool,
  modified to accept "raw lambdabox" *and* perform unboxing in the extraction step to Malfunction,
  is installed.
  The modified `peregrine` source code is available at https://github.com/simon-dima/lambda-box-extraction/tree/unbox.
...
Each backend uses one switch for `peregrine` and one for `malfunction` switch;
For example, `malfunction-reference` refers to `PEREGRINE_UNBOX_SWITCH` and `MALFUNCTION_FLAMBDA_SWITCH`,
but does not require the switches `PEREGRINE_NO_UNBOX_SWITCH` or `MALFUNCTION_NOFLAMBDA_SWITCH` to exist.
