namespace UnionFindNoInline

def StateT' (m : Type → Type) (σ : Type) (α : Type) := σ → m (α × σ)
namespace StateT'
variable {m : Type → Type} [Monad m] {σ : Type} {α β : Type}
protected def pure (a : α) : StateT' m σ α := fun s => pure (a, s)
protected def bind (x : StateT' m σ α) (f : α → StateT' m σ β) : StateT' m σ β := fun s => do let (a, s') ← x s; f a s'
def read : StateT' m σ σ := fun s => pure (s, s)
def write (s' : σ) : StateT' m σ Unit := fun _ => pure ((), s')
def updt (f : σ → σ) : StateT' m σ Unit := fun s => pure ((), f s)
...
protected def pure (a : α) : ExceptT' m ε α := (pure (Except.ok a) : m (Except ε α))
protected def bind (x : ExceptT' m ε α) (f : α → ExceptT' m ε β) : ExceptT' m ε β :=
(do { let v ← x; match v with
       | Except.error e => pure (Except.error e)
       | Except.ok a    => f a } : m (Except ε β))
def error (e : ε) : ExceptT' m ε α := (pure (Except.error e) : m (Except ε α))
def lift (x : m α) : ExceptT' m ε α := (do {let a ← x; pure (Except.ok a) } : m (Except ε α))
...
def read : M ufData := ExceptT'.lift StateT'.read
def write (s : ufData) : M Unit := ExceptT'.lift (StateT'.write s)
def updt (f : ufData → ufData) : M Unit := ExceptT'.lift (StateT'.updt f)
def error {α : Type} (e : Exn) : M α := ExceptT'.error e
...
end UnionFindNoInline

open UnionFindNoInline

def unionfind_noinline (n: Nat) : Nat :=
